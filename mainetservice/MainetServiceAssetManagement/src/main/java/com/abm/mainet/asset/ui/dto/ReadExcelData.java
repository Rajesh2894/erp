    ...
    private Object getCellDataInObject(PropertyDescriptor objPropertyDescriptor, Object obj, String colName, int rowNum)
        ...
        if (cell != null && !cell.toString().isEmpty()) {
            if (objPropertyDescriptor.getPropertyType() == String.class) {
                ...
                checkEmptyRow = 1;
                objPropertyDescriptor.getWriteMethod().invoke(obj, cell.getDateCellValue());
            ...
            try {
                ...
                for (Cell row : row) {
                    for (Map.Entry<String, String> entry : map.entrySet()) {
                        if (entry.getValue().trim().equals(row.toString().trim())) {
                            try {
                                rowError = row;
                ...
                LOGGER.error("Error: Verify Header Column " + ie);
                break;
            } catch (Exception e) {
