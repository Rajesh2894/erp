...
@Controller
@RequestMapping("/DeactivatedProperty.html")
public class DeactivatedPropertyReportController extends AbstractFormController<DetailOutstandingRegisterModel> {
	...
	@RequestMapping(params = "GetDeactivatedProperty", method = { RequestMethod.POST, RequestMethod.GET })
	public @ResponseBody String viewReportSheet(@RequestParam("wardZone1") Long wardZone1,
			...
			@RequestParam("propertyNo") String propertyNo, final HttpServletRequest request) {
		...
		if (error == null) {
			return ServiceEndpoints.PROPERTY_BIRT_REPORT_URL
					+ "=DeactivatedPropertyReport.rptdesign&__title=&false&OrgId=" + currentOrgId + "&WZoneB1="
					+ wardZone1 + "&WZoneB2=" + wardZone2 + "&WZoneB3=" + wardZone3 + "&WZoneB4=" + wardZone4
					+ "&WZoneB5=" + wardZone5 + "&FromDate=" + fromdt + "&ToDate=" + todt + "&PropertyNumber="
					+ propertyNo;
