...
import com.abm.mainet.property.rest.dto.PropertyDashboardMasterDTO;
...
@RestController
@RequestMapping("/propertyDashboardApi")
public class PropertyDashboardRestController {

	@SuppressWarnings("unchecked")
	
	private static final Logger LOGGER = Logger.getLogger(PropertyDashboardRestController.class);
	@RequestMapping(value = "/getPropDashboardDetails", method = RequestMethod.POST)
	public ResponseEntity<Object> getPropertyDashboardDetails() {
		...
		if(lookUp.getOtherField().equals(MainetConstants.FlagY)) {
			...
			LOGGER.info("Property API Dates List:"+totalDates);
		}else {
		...
		if(!totalDates.isEmpty() || totalDates!=null) {
			for (LocalDate locD : totalDates) {
				...
				PropertyDashboardMasterDTO req = new PropertyDashboardMasterDTO();
				PropertyDashboardRequestInfoDTO reqInfo = new PropertyDashboardRequestInfoDTO();
				...
				try {
					...
					ResponseEntity<?> response2 = getPropertyDashboardResponseDTO(date1);
					List<PropertyDashboardResponseDTO> data11 = (List<PropertyDashboardResponseDTO>) response2.getBody();
					...
					String url1 = ApplicationSession.getInstance().getMessage("prop.api.searchUrl");
					...
					LOGGER.info("Response from getPropertyDashboardDetails ------>" + response);
					
				} catch (KeyManagementException e1) {
					// TODO Auto-generated catch block
					LOGGER.error("error at the time of API getPropertyDashboardDetails call   " + e1);
					e1.printStackTrace();
					...
					LOGGER.error("error at the time of API getPropertyDashboardDetails call   " + e);
					e.printStackTrace();
				}

				LOGGER.info("Property API RequestPayload received for date--->"+locD+"--->"+response);
			}
		}
		LOGGER.info("Property API Data Ingestion succesfully completed for the given dates.");
		return response;
	}

	@ResponseBody
	private ResponseEntity<?> getPropertyDashboardResponseDTO(Date date1) {
		RestTemplate restTemplate;
		HttpHeaders headers = new HttpHeaders();
		ResponseEntity<?> responseEntity = null;
		PropertyDashboardRequestDTO requestDto = new PropertyDashboardRequestDTO();
		try {
			restTemplate = restTemplate();
			String url1 = ApplicationSession.getInstance().getMessage("prop.api.propertyDashboard.Url");
			LOGGER.info(" url1 Value ---------------->" + url1);
			...
			LOGGER.info("Response from getPropertyDashboardResponseDTO for DATA------>" + responseEntity);
