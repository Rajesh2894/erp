...
@Component("bndDeathCertificateApprovaModel")
...
public class DeathCertificateApprovalModel extends AbstractFormModel{
	...
	private List<AttachDocs> deAttDocsList = new ArrayList<>();
	...
	public boolean saveDeathApprovalDetails(String applicationNo, Long orgId, String task) {
		...
		if(Utility.isEnvPrefixAvailable(UserSession.getCurrent().getOrganisation(), MainetConstants.ENV_ASCL)) {
			prepareContractDocumentsData(deathCertificateDTO);
			}
		if (checkFinalAproval == true) {
			...
			if (attachments.isEmpty() || attachments == null) {
				...
				uploadDTO.setDepartmentName(BndConstants.RTS);
        ...
        requestDTO.setDepartmentName(BndConstants.RTS);
        ...
        List<Long> attacheMentIds = ApplicationContextProvider.getApplicationContext()
				...
				.fetchAttachmentIdByAppid(Long.valueOf(applicationNo), orgId);
        ...
        ServiceMaster service = ApplicationContextProvider.getApplicationContext().getBean(ServiceMasterService.class)
                .getServiceMasterByShortCode(BndConstants.RDC,UserSession.getCurrent().getOrganisation().getOrgid());
	...
	public void prepareContractDocumentsData(DeathCertificateDTO entity) {
		...
		requestDTO.setIdfId(entity.getApplicationNo().toString());
		requestDTO.setApplicationId(entity.getApplicationNo());
	...
	public List<AttachDocs> getDeAttDocsList() {
		return deAttDocsList;
	}

	public void setDeAttDocsList(List<AttachDocs> deAttDocsList) {
		this.deAttDocsList = deAttDocsList;
	}
