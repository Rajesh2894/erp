...
@Service
public class WorkEstimateApprovalAlertServiceImpl implements WorkEstimateApprovalAlertService {

	private static final Logger LOGGER = Logger.getLogger(WorkEstimateApprovalAlertServiceImpl.class);
	...
	/*
	 * Method used to send SMS and email to the high authority if the Work Estimate
	 * is not approved in stipulated time period
	 */
	@Transactional
	public void alertForWorkEstimateApproval(QuartzSchedulerMaster runtimeBean, List<Object> parameterList) {
		Long orgId = runtimeBean.getOrgId().getOrgid();
		// Organisation organisation=organisationService.getOrganisationById(orgId);
		// To get all WorkDefinitionDto those are Work Estimate approval cycle
		List<WorkDefinitionDto> workDefinitionDtos = workdefinationService
				.findAllWorkdefinationforScehedularByOrgId(orgId);

		workDefinitionDtos.forEach(dto -> {
			// To get WorkflowRequest base on the work code
			WorkflowRequest workflowRequest = ApplicationContextProvider.getApplicationContext()
					.getBean(IWorkflowRequestService.class)
					.getWorkflowRequestByAppIdOrRefId(null, dto.getWorkcode(), dto.getOrgId());
							...
							if (lookUp != null) {
								if (lookUp.getLookUpCode().equals(MainetConstants.FlagD)) {
									...
									try {
										...
										// Hour difference between last decision date and current date
										if (dateDifference == 0) {
	...
	private void sendMessageAndEmail(SMSAndEmailDTO smsAndEmailDTO, Long orgId) {
		...
		String menuUrl = "WorkEstimate.html";
