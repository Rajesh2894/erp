 ...
 */
@Controller
@RequestMapping("/TenderAward.html")
public class TenderAwardController extends AbstractFormController<TenderInitiationModel> {
	...
	public ModelAndView index(final Model model, HttpServletRequest httpServletRequest) {
		...
		if (master != null) {
			...
			String status = CommonMasterUtility.getNonHierarchicalLookUpObject(master.getSmServActive(), organisation)
					.getLookUpDesc();
	...
	public ModelAndView openEstimationForm(HttpServletRequest httpServletRequest,
		...
		getModel().bind(httpServletRequest);
		httpServletRequest.getSession().setAttribute(MainetConstants.WorksManagement.SAVEMODE,
				MainetConstants.WorksManagement.TNDR);
