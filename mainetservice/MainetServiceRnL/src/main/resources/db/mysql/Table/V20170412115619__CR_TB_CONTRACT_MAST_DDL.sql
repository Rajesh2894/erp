...
CREATE TABLE `TB_CONTRACT_MAST` (
  `CONT_ID` bigint(12) NOT NULL COMMENT 'primary key',
  `CONT_DEPT` bigint(12) NOT NULL COMMENT 'Contract Department(department selected in Party1 details)',
  `CONT_NO` varchar(40) NOT NULL COMMENT 'Contract No. (ORGID+DEPTCODE+FINANCIAL YEAR+999)',
  ...
  `CONT_RENEWAL` char(1) NOT NULL COMMENT 'Renewal of Contract (''Y'',''N)',
  ...
  `LANG_ID` int(11) NOT NULL COMMENT 'language identity',
  ...
  `cont_map_flag` char(1) DEFAULT NULL,
  PRIMARY KEY (`CONT_ID`)
) COMMENT='Table for Contract Creation,Contract renewal,contract revisi';
