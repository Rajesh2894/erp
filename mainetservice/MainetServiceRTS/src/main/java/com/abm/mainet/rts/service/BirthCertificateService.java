...
import com.abm.mainet.rts.repository.BirthCertificateRepository;
...
@WebService(endpointInterface = "com.abm.mainet.rts.service.IBirthCertificateService")
...
public class BirthCertificateService implements IBirthCertificateService {
	...
	public BirthCertificateDTO saveBirthCertificate(BirthCertificateDTO birthCertificateDto,
		...
		RTSBirthCertificateEntity birthcer = new RTSBirthCertificateEntity();
		final RequestDTO commonRequest = model.getRequestDTO();
		ServiceMaster serviceMas = iServiceMasterDAO.getServiceMasterByShortCode(RtsConstants.RBC, birthCertificateDto.getOrgId());
		...
		if (processName == null) {
			...
			birthCertificateDto.setBirthWfStatus(RtsConstants.APPROVED);
		}
		else {
			birthCertificateDto.setBrStatus(RtsConstants.STATUSOPEN);
			birthCertificateDto.setBirthWfStatus(RtsConstants.PENDING);
		}
	...
	private void smsAndEmail(BirthCertificateDTO birthCertificateDto,Organisation organisation)
	{
		...
		ApplicationContextProvider.getApplicationContext().getBean(ISMSAndEmailService.class).sendEmailSMS(
				RtsConstants.RTS, RtsConstants.APPLY_FOR_BIRTH_CERTIFICATE, PrefixConstants.SMS_EMAIL_ALERT_TYPE.GENERAL_MSG, smdto, organisation, birthCertificateDto.getLangId());
	...
	public BirthCertificateDTO saveBirthCertificates(BirthCertificateDTO birthCertificateDto) {
		...
		List<TbTaxMasEntity> taxList =taxMasService.fetchAllApplicableServiceCharge(serviceMas.getSmServiceId(), birthCertificateDto.getOrgId(), birthCertificateDto.getChargeApplicableAt());
		if(!taxList.isEmpty())
		{
		birthCertificateDto.getChargesInfo().get(0).setChargeCode(taxList.get(0).getTaxId());
		birthCertificateDto.getChargesInfo().get(0).setTaxCode(taxList.get(0).getTaxCode());
		}
		 if (serviceMas.getSmFeesSchedule() == 0) {
		initializeWorkFlowForFreeService(birthCertificateDto);
		 }
	...
	public void initializeWorkFlowForFreeService(BirthCertificateDTO requestDto) {
		...
		ServiceMaster serviceMas = iServiceMasterDAO.getServiceMasterByShortCode(RtsConstants.RBC, requestDto.getOrgId());
	...
	public BirthCertificateDTO getBirthRegisteredAppliDetail(Long applicationId, Long orgId) {
		RTSBirthCertificateEntity DetailEntity = birthCerRepro.findData(applicationId, orgId);
	...
	public void setAndSaveChallanDtoOffLine(CommonChallanDTO offline, BirthCertificateModel birthRegModel) {
		ServiceMaster serviceMas = iServiceMasterDAO.getServiceMasterByShortCode(RtsConstants.RBC,
		...
		/*
		 * if (birthRegModel.getBirthCertificateDto().getPdFathername() != null) {
		 * offline.setApplicantName(birthRegModel.getBirthCertificateDto().
		 * getPdFathername()); }
		 */
		
		String applicantAddress = String.join(" ", Arrays.asList(birthRegModel.getRequestDTO().getBldgName(),
				birthRegModel.getRequestDTO().getBlockName(), birthRegModel.getRequestDTO().getRoadName(), birthRegModel.getRequestDTO().getCityName()));
	...
	public void smsAndEmailApproval(BirthCertificateDTO birthCertificateDto ,String decision) {
			...
			smdto.setEmail(birthCertificateDto.getApplicantEmail());
			ApplicationContextProvider.getApplicationContext().getBean(ISMSAndEmailService.class).sendEmailSMS(
					RtsConstants.RTS, "BirthCertificateApproval.html", PrefixConstants.SMS_EMAIL_ALERT_TYPE.GENERAL_MSG, smdto, organisation, birthCertificateDto.getLangId());
	...
	public BirthCertificateDTO getBirthRegisteredAppliDetails(BirthCertificateDTO birthCertificateDto) {
		RTSBirthCertificateEntity DetailEntity = birthCerRepro.findData(birthCertificateDto.getApmApplicationId(), birthCertificateDto.getOrgId());
		BirthCertificateDTO birthDto = new BirthCertificateDTO();
		BeanUtils.copyProperties(DetailEntity, birthDto);
		return birthDto;
	}
