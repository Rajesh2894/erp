 ...
 */
@Repository
public class VehicleFuelDAO extends AbstractDAO<VehicleFuellingData> implements IVehicleFuellingDAO {
    ...
    @Override
    public List<VehicleFuellingData> searchVehicleFuelling(Long vehicleType, Long pumpId, Date fromDate,
    ...
    @SuppressWarnings("unchecked")
    @Override
    public List<VehicleFuellingData> getVehicleFuellingByAdviceDateAndPumpId(Long pumpId, Date fromDate,
    ...
    private String buildQuery(Long vehicleType, Long pumpId, Date fromDate, Date toDate) {
        StringBuilder searchQuery = new StringBuilder(
                " SELECT vf FROM VehicleFuellingData vf WHERE vf.orgid = :orgid ");
    ...
    private String buildAdviceQuery(Long pumpId, Date fromDate, Date toDate, Boolean paid) {
        ...
        if (paid) {
            searchQuery.append(" AND vf.vefId  NOT IN (SELECT d.vefId FROM VehicleFuelReconciationDetails d WHERE d.orgid = :orgid)");
        } else {
            searchQuery.append(" AND vf.vefId  IN (SELECT d.vefId FROM VehicleFuelReconciationDetails d WHERE d.orgid = :orgid)");
        }
