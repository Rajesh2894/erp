 ...
 */
@Repository
public class FuelPumpMasterDAO extends AbstractDAO<FuelPumpMaster> implements IPumpMasterDAO {
    ...
    @Override
    public List<FuelPumpMaster> searchPumpByPumpType(String status, Long pumpType, Long puId, String pumpName, Long orgId) {
    ...
    private String buildQuery(String status, Long pumpType, Long puId, String pumpName) {
        StringBuilder pumpMasterSearchQuery = new StringBuilder(
                " SELECT pm FROM FuelPumpMaster pm WHERE pm.orgid = :orgid ");
        ...
        if (StringUtils.isNotBlank(pumpName)) {
            pumpMasterSearchQuery.append(" AND pm.puPumpname like concat('%',:pumpName,'%')");
        }
