...
@Controller
@RequestMapping("/AccountTenderEntryAuthorization.html")
public class AccountTenderEntryAuthorizationController extends AbstractController {

    /**
     * @param controllerClass
     * @param entityName
     */
    public AccountTenderEntryAuthorizationController() {
        super(AccountTenderEntryAuthorizationController.class, MainetConstants.CommonConstant.BLANK);

    }

    private final String JSP_FORM = "AccountTenderEntryAuthorization/form";
    private final String JSP_LIST = "AccountTenderEntryAuthorization/list";
    private final String SAVE_ACTION_CREATE = "/AccountTenderEntryAuthorization/create";
    private final String SAVE_ACTION_UPDATE = "/AccountTenderEntryAuthorization/create";
    private final String CHECKLIST_FORM = "AccountTenderAuthorization/form";
        ...
        for (final AccountTenderEntryBean accountTenderEntryBean : chList) {
            ...
            for (final AccountTenderDetBean accountTenderDetBean : prBudgetCodeIdList) {
                ...
                if (prBudgetCodeId != null) {
                    final String budgetCode = tbAcSecondaryheadMasterService
                            .getAccountHeadCodeInReceieptDetEntry(prBudgetCodeId);
    ...
    public @ResponseBody List getCheqData(final HttpServletRequest request, final Model model,
            @RequestParam("trTenderNo") final String trTenderNo, @RequestParam("vmVendorid") final Long vmVendorid,
            @RequestParam("trTypeCpdId") final Long trTypeCpdId, @RequestParam("sacHeadId") final Long sacHeadId,
            @RequestParam("trTenderAmount") final String trTenderAmount,
        ...
        chList = new ArrayList<>();
        chList.clear();
        chList = tbAcTenderEntryService.findByAllGridSearchData(trTenderNo, vmVendorid, trTypeCpdId, sacHeadId,
                trTenderAmount, statusId, orgId);
        return chList;
    }

    public void populateListOfTenderType(final Model model) {

        final List<LookUp> tenderTypeLookUp = CommonMasterUtility.getListLookup(AccountPrefix.LTY.toString(),
    ...
    public String formForUpdate(final Model model, final AccountTenderEntryBean tenderEntryBean,
        ...
        final AccountTenderEntryEntity tenderEntryList = tbAcTenderEntryService.findById(trTenderId);
        ...
        tenderEntryBean.setDpDeptid(tenderEntryList.getTbDepartment().getDpDeptid());
        tenderEntryBean.setTrTypeCpdId(tenderEntryList.getTbComparamDet().getCpdId());
        tenderEntryBean.setTrTenderAmount(tenderEntryList.getTrTenderAmount().toString());
        if (tenderEntryList.getTrEmdAmt() != null) {
            tenderEntryBean.setTrEmdAmt(tenderEntryList.getTrEmdAmt().toString());
        }
        tenderEntryBean.setVmVendorid(tenderEntryList.getTbVendormaster().getVmVendorid());
        ...
        tenderEntryBean.setSpecialconditions(tenderEntryList.getSpecialconditions());

        tenderEntryBean.setAuthStatus(tenderEntryList.getAuthStatus());
        if ((tenderEntryList.getAuthRemark() != null) && !tenderEntryList.getAuthRemark().isEmpty()) {
            tenderEntryBean.setAuthRemark(tenderEntryList.getAuthRemark());
        }
        ...
        final Long emdId = CommonMasterUtility.lookUpIdByLookUpCodeAndPrefix(
                PrefixConstants.TenderEntryAuthorization.EM, AccountPrefix.DTY.toString(), orgId);
    ...
    public String create(@Valid final AccountTenderEntryBean tenderEntryBean, final BindingResult bindingResult,
            final Model model, final RedirectAttributes redirectAttributes, final HttpServletRequest httpRequestMapping)
            throws Exception {
        log("Action,'create'");

        if (!bindingResult.hasErrors()) {
            tenderEntryBean.setHasError(MainetConstants.BUG_HEAD_OPENING_BALANCE_MASTER.TRUE);

            tenderEntryBean.setOrgid(UserSession.getCurrent().getOrganisation().getOrgid());
            ...
            tenderEntryBean.setCreatedBy(UserSession.getCurrent().getEmployee().getEmpId());
            ...
            model.addAttribute(MainetConstants.TENDER_ENTRY.TENDER_ENTRY_BEAN, tbAcTenderEntryService);
            ...
            tenderEntryBean.setHasError(MainetConstants.BUG_HEAD_OPENING_BALANCE_MASTER.FALSE);
            final List<AccountTenderDetBean> list = keepDataAfterValidationError(tenderEntryBean, bindingResult);
     ...
     * @return
     */
    private List<AccountTenderDetBean> keepDataAfterValidationError(final AccountTenderEntryBean tenderEntryBean,
    ...
    private void populateModel(final Model model, final AccountTenderEntryBean tenderEntryBean, final FormMode formMode)
            throws Exception {
        model.addAttribute(MainetConstants.TENDER_ENTRY.ACCOUNT_TENDER_ENTRY, tenderEntryBean);
        if (formMode == FormMode.CREATE) {
            ...
            model.addAttribute(MainetConstants.TENDER_ENTRY.DEPT_MAP, deptMap);
    ...
    public @ResponseBody Map<String, String> findBudgetExpAmountData(final AccountTenderEntryBean bean,
        ...
        Long yearId = tbFinancialyearService
                .getFinanciaYearIdByFromDate(Utility.stringToDate(bean.getTrTenderDate()));
        ...
        for (AccountBudgetProjectedExpenditureEntity accountBudgetProjectedExpenditureEntity : projectedExpList) {
            if (accountBudgetProjectedExpenditureEntity.getRevisedEstamt() == null
                ...
                if (accountBudgetProjectedExpenditureEntity.getExpenditureAmt() == null) {
                    ...
                    BigDecimal expAmt = new BigDecimal(
                            accountBudgetProjectedExpenditureEntity.getExpenditureAmt().toString());
    ...
    private @ResponseBody List<AccountTenderDetBean> populateListOfAccountBudgetgetProjectedExpenditureItems(
            ...
            @RequestParam("pacHeadId") final Long pacHeadId, @RequestParam("sacHeadId") final Long sacHeadid,
            @RequestParam("fundId") final Long fundId, @RequestParam("fieldId") final Long feildId,
        ...
        final Date date = UtilityService.convertStringDateToDateFormat(tenderEntryDate);
    ...
    private void populateVendorList(final Model model) {
        ...
        if ((vendorList != null) && !vendorList.isEmpty()) {
            for (final TbAcVendormaster vendor : vendorList) {
                vendorName = vendor.getVmVendorcode() + MainetConstants.SEPARATOR + vendor.getVmVendorname();
    ...
    public String showDetails(final Model model, final HttpServletRequest httpServletRequest,
            final AccountTenderEntryBean tenderEntryBean, @RequestParam("appNo") final String appNo,
        ...
        final Long orgId = UserSession.getCurrent().getOrganisation().getOrgid();
        final AccountTenderEntryEntity tenderEntryList = tbAcTenderEntryService.findByTenderNo(appNo, orgId);
        populateModel(model, tenderEntryBean, FormMode.UPDATE);
        tenderEntryBean.setAppNo(appNo.toString());
        tenderEntryBean.setActualTaskId(Long.valueOf(actualTaskId).toString());
        tenderEntryBean.setTrTenderId(tenderEntryList.getTrTenderId());
        tenderEntryBean.setTrTenderNo(tenderEntryList.getTrTenderNo());
        tenderEntryBean.setTrTenderDate(UtilityService.convertDateToDDMMYYYY(tenderEntryList.getTrTenderDate()));
        tenderEntryBean.setDpDeptid(tenderEntryList.getTbDepartment().getDpDeptid());
        ...
        tenderEntryBean.setTrProposalNo(tenderEntryList.getTrProposalNo());
        tenderEntryBean
                .setTrProposalDate(UtilityService.convertDateToDDMMYYYY(tenderEntryList.getTrProposalDate()));
        tenderEntryBean.setSpecialconditions(tenderEntryList.getSpecialconditions());
    ...
    @RequestMapping(params = "workFlowAuthorizationTender")
    public String workOrderAuthorizationTender(@Valid final AccountTenderEntryBean tenderEntryBean,
            final BindingResult bindingResult,
            final Model model, final RedirectAttributes redirectAttributes, final HttpServletRequest httpRequestMapping)
            throws Exception {
        log("Action,'workOrderAuthorization'");
        if (!bindingResult.hasErrors()) {
            tenderEntryBean.setHasError(MainetConstants.BUG_HEAD_OPENING_BALANCE_MASTER.TRUE);
            tenderEntryBean.setOrgid(UserSession.getCurrent().getOrganisation().getOrgid());
            tenderEntryBean.setOrgShortName(UserSession.getCurrent().getOrganisation().getOrgShortNm());
            ...
            tenderEntryBean.setCreatedDate((new Date()));
            tenderEntryBean.setLgIpMac(Utility.getClientIpAddress(httpRequestMapping));
            tenderEntryBean.setWorkFlowIdentityFlag(MainetConstants.Y_FLAG);
