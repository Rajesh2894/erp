...
@Component
@Scope("session")
public class LicenseApprovalReprintingModel extends AbstractFormModel {
	...
	public boolean saveForm() {
		...
		masDto.getTradeLicenseOwnerdetailDTO().forEach(ownDto -> {
			if (ownDto.getCreatedBy() == null) {
				...
				if (!ownDto.getTroPr().equalsIgnoreCase(MainetConstants.FlagD)
						...
						&& !ownDto.getTroPr().equalsIgnoreCase(MainetConstants.FlagN)
						&& !ownDto.getTroPr().equalsIgnoreCase(MainetConstants.FlagR)) {
					ownDto.setTroPr(MainetConstants.FlagA);
				} else if (ownDto.getTroPr().equalsIgnoreCase(MainetConstants.FlagI)) {
					ownDto.setTroPr(MainetConstants.FlagN);
				}
				// add for setting I-Inactivate previous owner details defect#113170
				else if (ownDto.getTroPr().equalsIgnoreCase(MainetConstants.FlagD)) {
		...
		if(Utility.isEnvPrefixAvailable(UserSession.getCurrent().getOrganisation(),
			...
			this.setSuccessMessage(getAppSession().getMessage("report.licenseNo")
					+ MainetConstants.WHITE_SPACE + masDto.getTrdLicno() 
					+ MainetConstants.WHITE_SPACE + getAppSession().getMessage("trade.transfer.approval.succuess"));
	    }else {
	...
	private void sendSmsEmail(TradeMasterDetailDTO masDto) {
		...
		ServiceMaster sm = ApplicationContextProvider.getApplicationContext().getBean(ServiceMasterService.class)
				.getServiceMasterByShortCode(MainetConstants.TradeLicense.TRANSFER_SERVICE_SHORT_CODE,
						UserSession.getCurrent().getOrganisation().getOrgid());
		setServiceMaster(sm);
		smsDto.setServName(sm.getSmServiceName());
		smsDto.setEmail(masDto.getTradeLicenseOwnerdetailDTO().get(0).getTroEmailid());
		String url = "TransferLicenseApprovalReprinting.html";
