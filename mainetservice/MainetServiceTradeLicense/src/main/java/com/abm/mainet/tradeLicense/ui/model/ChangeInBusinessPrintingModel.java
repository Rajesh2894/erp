...
@Component
@Scope("session")
public class ChangeInBusinessPrintingModel extends AbstractFormModel {
	...
	public boolean saveForm() {
		...
		if(masDto.getPmPropNo() != null ) {
			...
			this.sendSmsEmail(trdDto, "ChangeInBusinessNameForm.html",PrefixConstants.SMS_EMAIL_ALERT_TYPE.GENERAL_MSG);
		}
		return true;
	}
	...
	private void sendSmsEmail(TradeMasterDetailDTO trdDto, String menuUrl, String msgType) {
		Long orgId= UserSession.getCurrent().getOrganisation().getOrgid();		
		ServiceMaster sm = serviceMasterService.getServiceByShortName(MainetConstants.TradeLicense.CHANGE_BUSINESS_NAME_SHORT_CODE,orgId);
	...
	private void sendSmsEmail(TradeMasterDetailDTO masDto) {
		...
		String url = "ChangeInBusinessPrinting.html";
