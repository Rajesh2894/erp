...
DELIMITER $$
CREATE DEFINER=`dev`@`192.168.100.15` PROCEDURE `UPSUDAPORTAL`()
BEGIN
    ...
    declare cu_orgid cursor for select orgid from suda_portal1.tb_organisation where orgid>=14;
...
insert into suda_portal1.tb_group_mast
...
UPDATED_BY,UPDATED_DATE,LG_IP_MAC_UPD,GR_DEFAULT from suda_portal1.tb_group_mast where orgid=1); 

insert into suda_portal1.employee
...
(select GM_ID from suda_portal1.tb_group_mast where GR_CODE='SUPER_ADMIN' and orgid=n_orgid),
n_orgid, 
CREATED_BY,CREATED_DATE from suda_service1.employee where orgid=n_orgid and EMPLOGINNAME='SysAdmin'); 
...
CREATED_BY,CREATED_DATE from suda_portal1.employee where orgid=1 and empname='GUEST');
...
b.DP_DEPTDESC,
b.DP_NAME_MAR
from suda_service1.tb_services_mst a,
suda_service1.tb_department b
