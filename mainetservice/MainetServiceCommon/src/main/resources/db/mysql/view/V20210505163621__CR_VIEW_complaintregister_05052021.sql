...
--changeset Kanchan:V20210505163621__CR_VIEW_complaintregister_05052021.sql
drop view if exists complaintregister;
--liquibase formatted sql
--changeset Kanchan:V20210505163621__CR_VIEW_complaintregister_050520211.sql
CREATE
    ALGORITHM = UNDEFINED
    DEFINER = dev@192.168.100.162
...
VIEW complaintregister AS
    SELECT
        ...
        (CASE
            WHEN
                ((REPLACE(b.STATUS, 'EXPIRED', 'CLOSED') = 'CLOSED')
            ...
            THEN
                'CLOSED'
            WHEN
                ((REPLACE(b.STATUS, 'EXPIRED', 'CLOSED') = 'PENDING')
        ...
        (CASE
            ...
            THEN
                'W'
            WHEN
                ((REPLACE(b.STATUS, 'EXPIRED', 'CLOSED') IN ('CLOSED' , 'EXPIRED'))
                    AND (TIMESTAMPDIFF(SECOND,
                    b.DATE_OF_REQUEST,
