...
--changeset nilima:V20190225200816__vw_complaintregister.sql
drop view if exists complaintregister;
--liquibase formatted sql
--changeset nilima:V20190225200816__vw_complaintregister1.sql
    ...
    SELECT 
        ...
        (SELECT 
            ...
            FROM
                tb_comparam_det
            WHERE
                (CPD_ID = h.ORG_CPD_ID_DIV)) AS DIVISION_ENG,
        (SELECT 
                CPD_DESC_MAR
            FROM
                tb_comparam_det
            WHERE
                (CPD_ID = h.ORG_CPD_ID_DIV)) AS DIVISION_REG,
        ...
        (SELECT 
            ...
            FROM
                tb_comparam_det
            WHERE
                (CPD_ID = h.ORG_CPD_ID_DIS)) AS DISTRICT_ENG,
        (SELECT 
                CPD_DESC_MAR
            FROM
                tb_comparam_det
            WHERE
                (CPD_ID = h.ORG_CPD_ID_DIS)) AS DISTRICT_REG,
        (SELECT 
                CPD_DESC
            FROM
                tb_comparam_det
            WHERE
                (CPD_ID = h.ORG_CPD_ID)) AS ULB_TYPE,
        a.APM_APPLICATION_ID AS APM_APPLICATION_ID,
		
        a.COMPLAINT_DESC AS COMPLAINTDESC,
        (SELECT APA_MOBILNO FROM tb_cfc_application_address WHERE APM_APPLICATION_ID = a.APM_APPLICATION_ID) AS APA_MOBILNO,
        (SELECT CONCAT(CONCAT(CONCAT(APM_FNAME,' '),APM_MNAME,' '),APM_LNAME) FROM tb_cfc_application_mst WHERE APM_APPLICATION_ID = a.APM_APPLICATION_ID) AS APPLICANT_NAME,
        ...
        a.LOC_ID AS LOC_ID,
        (SELECT LOC_NAME_ENG FROM tb_location_mas WHERE LOC_ID = a.LOC_ID) AS LOCATIONNAME_ENG,
        (SELECT LOC_NAME_REG FROM tb_location_mas WHERE LOC_ID = a.LOC_ID) AS LOCATIONNAME_REG,
        (SELECT LATTIUDE FROM tb_location_mas WHERE LOC_ID = a.LOC_ID) AS Latitude,
        (SELECT LONGITUDE FROM tb_location_mas WHERE LOC_ID = a.LOC_ID) AS longitude,
		(SELECT pincode FROM tb_location_mas WHERE LOC_ID = a.LOC_ID) AS pincode,
        g.COD_ID_OPER_LEVEL1 AS COD_ID_OPER_LEVEL1,
        (SELECT COD_DESC FROM
                tb_comparent_det
            WHERE
                (COD_ID = g.COD_ID_OPER_LEVEL1)) AS L1,
        ...
        (SELECT 
            ...
            FROM
                tb_comparent_det
            WHERE
                (COD_ID = g.COD_ID_OPER_LEVEL2)) AS L2,
        ...
        (SELECT 
            ...
            FROM
                tb_comparent_det
            WHERE
                (COD_ID = g.COD_ID_OPER_LEVEL3)) AS L3,
        g.COD_ID_OPER_LEVEL4 AS COD_ID_OPER_LEVEL4,
        (SELECT COD_DESC FROM
                tb_comparent_det
            WHERE
                (COD_ID = g.COD_ID_OPER_LEVEL3)) AS L4,
        ...
        (SELECT 
            ...
            FROM
                tb_comparent_det
            WHERE
                (COD_ID = g.COD_ID_OPER_LEVEL5)) AS L5,
        ...
        (SELECT 
            ...
            FROM
                tb_services_mst
            WHERE
                (SM_SERVICE_ID = e.SM_SERVICE_ID)) AS SERVICE_TYPE,
        (SELECT 
                b.COD_DESC
            FROM
        ...
        (SELECT 
            ...
            FROM
                tb_care_feedback
            WHERE
                (TOKEN_NUMBER = a.APM_APPLICATION_ID)) AS FEEDBACKRATINGS,
        (SELECT 
                RATINGS_STAR_COUNT
            FROM
                tb_care_feedback
            WHERE
                (TOKEN_NUMBER = a.APM_APPLICATION_ID)) AS FEEDBACKSTARCOUNT,
        IFNULL((SELECT 
                        CPD_DESC
                    FROM
                        tb_comparam_det
                    WHERE
                        (CPD_ID = a.REFERENCE_MODE)),
                'ONLINE') AS CARE_MODE,
        ...
        (CASE
            WHEN
                ((b.STATUS = 'PENDING')
                    ...
                    WHEN
                        (((SELECT 
                                SUM(COALESCE(SLA_CAL, 0))
                            FROM
                                tb_workflow_det
                            WHERE
                                (WF_ID = e.WF_ID)) / 1000) <> 0)
                    THEN
                        ((SELECT 
                                SUM(COALESCE(SLA_CAL, 0))
                            FROM
        ...
        (SELECT 
            ...
            WHERE
                (APM_APPLICATION_ID = a.APM_APPLICATION_ID)) AS APA_EMAIL
    FROM
        (((((((tb_care_request a
