...
--changeset Kanchan:V20210811194718__CR_VIEW_complaintregister_11082021.sql
drop view if exists complaintregister;
--liquibase formatted sql
--changeset Kanchan:V20210811194718__CR_VIEW_complaintregister_110820211.sql
CREATE VIEW complaintregister AS
    SELECT 
        ...
        a.CARE_WARD_NO2 AS care_ward_no2,
        (SELECT 
                tb_comparent_det.COD_DESC
            FROM
                tb_comparent_det
            WHERE
                (tb_comparent_det.COD_ID = a.CARE_WARD_NO2)) AS CARE_WARD_NO2_ENG,
        (SELECT 
                tb_comparent_det.COD_DESC_MAR
            FROM
                tb_comparent_det
            WHERE
                (tb_comparent_det.COD_ID = a.CARE_WARD_NO2)) AS CARE_WARD_NO2_REG,
        a.CARE_WARD_NO3 AS care_ward_no3,
        (SELECT 
                tb_comparent_det.COD_DESC
            FROM
                tb_comparent_det
            WHERE
                (tb_comparent_det.COD_ID = a.CARE_WARD_NO3)) AS CARE_WARD_NO3_ENG,
        (SELECT 
                tb_comparent_det.COD_DESC_MAR
            FROM
                tb_comparent_det
            WHERE
                (tb_comparent_det.COD_ID = a.CARE_WARD_NO3)) AS CARE_WARD_NO3_REG,
        a.CARE_WARD_NO4 AS care_ward_no4,
        (SELECT 
                tb_comparent_det.COD_DESC
            FROM
                tb_comparent_det
            WHERE
                (tb_comparent_det.COD_ID = a.CARE_WARD_NO4)) AS CARE_WARD_NO4_ENG,
        (SELECT 
                tb_comparent_det.COD_DESC_MAR
            FROM
                tb_comparent_det
            WHERE
                (tb_comparent_det.COD_ID = a.CARE_WARD_NO4)) AS CARE_WARD_NO4_REG,
        (SELECT 
        ...
        (CASE
            WHEN
                ((b.STATUS = 'PENDING')
                    ...
                    WHEN
                        (((SELECT 
                                SUM(COALESCE(tb_workflow_det.SLA_CAL,
                                            0))
                            FROM
                                tb_workflow_det
                            WHERE
        ...
        (SELECT 
                tb_comparam_det.CPD_VALUE
            FROM
                tb_comparam_det
            WHERE
                (tb_comparam_det.CPD_ID = a.REFERENCE_MODE)) AS CARE_MODE_CODE
    FROM
        (((tb_care_request a
