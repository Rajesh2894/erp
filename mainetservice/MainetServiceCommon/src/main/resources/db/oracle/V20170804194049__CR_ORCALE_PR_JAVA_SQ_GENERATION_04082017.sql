CREATE OR REPLACE PROCEDURE PR_JAVA_SQ_GENERATION
(v_mod  IN   varchar2,             -- Module Name
 v_tbl  IN   varchar2,             -- Table Name
 v_fld   IN  varchar2,             -- Field Name
 v_rst   IN   varchar2,             -- Reset Type when first time created
 v_ctr_id IN varchar2 default null, -- For Counterwise Resetting and generating sequnce no. from 1 for each counter in receipt.
...
begin
  ...
  -- To check whether Table and Corresponding field are exist or not in Database
   ...
   IF (v_ctr_id is not null) THEN
       v_sqname_new      := 'SQ_'||UPPER(substr(v_field,1,6))||v_ctr_id||'_';
   else
