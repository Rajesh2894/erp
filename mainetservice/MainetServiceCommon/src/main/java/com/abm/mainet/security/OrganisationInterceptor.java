...
public class OrganisationInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(final HttpServletRequest request, final HttpServletResponse response, final Object handler)
		...
		if (request.getServletPath().contains("/Home.html")) {
			return true;
		}
		...
		if (isULBSet(request, Long.valueOf(orgId.trim()))) {
			...
			if (bypassUrl.contains(MainetConstants.operator.AMPERSAND)) {
				bypassUrl = bypassUrl.replaceAll(MainetConstants.operator.AMPERSAND,"@");
			}
			request.getRequestDispatcher("AdminHome.html?bypassUrl=" + bypassUrl).forward(request, response);
		}
		return false;
	}
	...
	private boolean isULBSet(final HttpServletRequest request, final Long orgId) {
		...
		if (request.getAttribute(ORGANIZATION_REQ_ATTR_NAME) != null) {
			...
			if ((organisation != null) && (organisation.getOrgid() == orgId.longValue())) {
				retVal = true;
			}
		}
		if (retVal) {
