...
public class PaymentBeanFactory {
	...
	public static PaymentStrategy getInstance(final String pgName) {
		...
		} else if ((pgName != null) && pgName.equalsIgnoreCase(MainetConstants.PG_SHORTNAME.BILLDESK)) {
			paymentStrategy = new BillDesk();
		} else if ((pgName != null) && pgName.equalsIgnoreCase(MainetConstants.PG_SHORTNAME.RAZORPAY)) {
			paymentStrategy = new Razorpay();
		} else if ((pgName != null) && pgName.equalsIgnoreCase(MainetConstants.PG_SHORTNAME.ATOMPAY)) {
			paymentStrategy = new AtomPayment();
		}else if ((pgName != null) && pgName.equalsIgnoreCase(MainetConstants.PG_SHORTNAME.EASEBUZZ)) {
		paymentStrategy = new Easebuzz();
		}
		else if ((pgName != null) && pgName.equalsIgnoreCase(MainetConstants.PG_SHORTNAME.NicGateway)) {
			paymentStrategy = new NicGateway();
			}
