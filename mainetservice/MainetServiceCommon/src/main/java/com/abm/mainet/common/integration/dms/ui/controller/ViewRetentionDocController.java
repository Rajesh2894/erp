...
import com.abm.mainet.common.integration.dms.service.IViewRetentionService;
...
@Controller
@RequestMapping(value = "ViewRetentionDetails.html")
public class ViewRetentionDocController extends AbstractFormController<ViewRetentionModel> {

	@Autowired
	private IViewRetentionService viewMetadataService;

	@RequestMapping(method = { RequestMethod.POST, RequestMethod.GET })
	public ModelAndView index(HttpServletRequest request, Model model) {
		sessionCleanup(request);
		return new ModelAndView("ViewRetentionDetails", MainetConstants.FORM_NAME, getModel());
	}
	...
	public ModelAndView searchMetadataDetails(@RequestParam("level1") Long deptId,
		...
		List<DmsRetentionDto> metadataList = viewMetadataService.getMetadataDetails(String.valueOf(deptId),
		...
		getModel().setDmsRetentionDtoList(metadataList);
		...
		if (getModel().getErrorMsg() != null && getModel().getErrorMsg().equals(MainetConstants.FlagY)) {
			mv = new ModelAndView("ViewRetentionDetailsValidn", MainetConstants.FORM_NAME, this.getModel());
			this.getModel().addValidationError(getApplicationSession().getMessage("dms.noRecordInService"));
			mv.addObject(BindingResult.MODEL_KEY_PREFIX + MainetConstants.FORM_NAME, getModel().getBindingResult());
			return mv;
		} else {
			return new ModelAndView("ViewRetentionDetailsValidn", MainetConstants.FORM_NAME, getModel());
		}
