...
<form:hidden path="" id="bifurcationId" value="${command.getSelfAss()}"/>
...
<div class="panel-collapse collapse in" id="UnitDetail">
		...
		<c:set var="d" value="0" scope="page" />			
                <table id="unitDetailTable" class="table table-striped table-bordered appendableClass unitDetails">
                    <tbody>                
                        <tr>
                        	<th width="12%" class="required-control"><spring:message code="unitdetails.year"/></th>
...
<%--                             <th width="14%" class="required-control"><spring:message code="property.changeInAss.EffectiveDate"/></th>           
 --%>                            <th width="16%" class="required-control"><spring:message code="unitdetails.constructiontype"/></th>
                     	 	<th width="12%" class="required-control"><spring:message code="" text="Select to bifurcate"/></th>        	 
                    	...
                    	<tr class="firstUnitRow">                     
                      		 <td>
                        	    ...
                        	    <form:select path="provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList[${status.count-1}].faYearId" id="year${status.count-1}" class="form-control disabled text-center mandColorClass" >
							...
							<td>
								...
								<form:input type="text" path="provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList[${status.count-1}].assdYearConstruction" class="form-control datepicker2 mandColorClass dateClass " id="yearOfConstruc${status.count-1}" data-rule-required="true" placeholder="DD/MM/YYYY" />									
						...
						<%-- 	<td>
                            ...
                            <td> 
								...
								<apptags:lookupField
									...
									<th width="120"><label class="checkbox-inline">
									
									<form:checkbox path="provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList[${status.count-1}].assdBifurcateNo" value="Y" cssClass="checkall" id="assdBifurcateNo" /> </label>
									<%-- <form:hidden path="provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList[${status.count-1}].assdBifurcateNo" id="checkValue"/> --%>
											<%-- <c:forEach items="${command.getProvisionalAssesmentMstDto().getProvisionalAssesmentDetailDtoList()}" varStatus="status1">
							</c:forEach> --%>
											</th>
									<%-- <td class="text-center">
	                         <form:select path="provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList[${status.count-1}].assdBifurcateNo" cssClass="form-control" id="assdBifurcateNo${status.count-1}">
	                         <form:option value="0">
								<spring:message code="adh.select" text="Select"></spring:message>
							</form:option>
							<c:forEach items="${command.getProvisionalAssesmentMstDto().getProvisionalAssesmentDetailDtoList()}" varStatus="status1">
							<form:option value="${status1.count}">${status1.count}</form:option>
							</c:forEach>
	                         </form:select>
	                         </td> --%>
							...
							<td width="150"><form:input path="provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList[${status.count-1}].assdBuildupArea" type="text" class="form-control has2Decimal mandColorClass text-right" id="taxableArea${status.count-1}" data-rule-required="true" onkeypress="return hasAmount(event, this, 15, 2)" onchange="getAmountFormatInDynamic((this),'proAssdBuildupArea')" placeholder="999999.99"/></td>
							 ...
							 <td class="text-center"><a href="javascript:void(0);" class="remCF btn btn-danger btn-xs delete" id="deleteRow_${status.count-1}"><i class="fa fa-minus-circle"></i></a></td>
