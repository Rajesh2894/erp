...
<div class="content">
	...
	<div class="widget-header">
				<h2><strong><spring:message code="property.selfassessment" text="Self Assessment"/></strong></h2>				
...
<!-- Start Widget Content -->
	<div class="widget-content padding">
		...
		<form:form action="SelfAssessmentForm.html"
			...
			<div class="form-group">
				   ...
				   <c:set var="baseLookupCode" value="SRT" />
							<apptags:lookupField
									...
									selectOptionLabelCode="unit.select"/>
			...
			<input type="hidden" value="${command.getAssType()}" id="assType"/>
			<c:if test="${command.getAssType() eq 'N' || command.getAssType() eq 'H'}">  
				<button type="button" class="btn btn-success"
				onclick="getCheckList(this)" id="checkList"><spring:message code="unit.proceed"/></button>
			</c:if>
			
			<c:if test="${command.getSelfAss() eq 'Y'}">  
				<input 	type="button" class="btn btn-warning" value="Reset" onclick="resetFormData(this)" id="ResetSelfAssForm"/>	
			</c:if>
															...
															<tr>
																...
																<td><div id="docs_${lk}" class="">
																		<apptags:formField fieldType="7" labelCode=""
																			...
																			<i style="font-size: 10px;font-weight: bold;" class="text-red-1"><spring:message code="" text="(Upload file upto 5MB and only pdf,doc,docx,xls,xlsx)"/></i>
																	</div>
																	</td>
															</tr>
														</c:forEach>
			...
			<button type="button" class="btn btn-success btn-submit"
				onclick="compareBeforeAfterAuth(this)" id="btnCompare">Proceed to Compare</button>
			</c:if>
			</div>
