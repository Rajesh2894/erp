...
<div class="widget">
 <div class="widget-header">
   <c:if test="${command.getAssType() eq 'A' || command.getAssType() eq 'DES' }">  
       <!--  <div class="widget-header">  -->
       <c:if test="${command.getAssType() eq 'A' || command.getAssType() eq 'DES' }">  
		<%-- 	<h2><strong><spring:message code="propertyView.DataEntryView" /></strong></h2>	 --%>
		</c:if> 
		...
		<form:form action="DataEntrySuite.html" class="form-horizontal form" name="DataEntrySuiteView" id="DataEntrySuiteView">	
...
<%-- 				<apptags:input labelCode="property.buildingpermission" path="" isDisabled="true"></apptags:input> 
 --%>				<apptags:input labelCode="propertydetails.PropertyNo." path="provisionalAssesmentMstDto.assNo" isDisabled="true"></apptags:input>
			...
			<c:if test="${command.provisionalAssesmentMstDto.parentPropName ne null}">
				<div class="form-group">
					<div class="col-sm-6">
				...
				<div class="form-group">
					<c:if test="${command.provisionalAssesmentMstDto.parentPropName ne null}">
						<apptags:input labelCode="property.parentPropName"
							path="provisionalAssesmentMstDto.parentPropName" isDisabled="true"></apptags:input>
					</c:if>
					<c:if test="${command.provisionalAssesmentMstDto.groupPropName ne null}">
						<apptags:input labelCode="property.groupPropName"
							path="provisionalAssesmentMstDto.groupPropName" isDisabled="true"></apptags:input>
					</c:if>
				</div>
			</c:if>
...
<!----------------------------------------------------------------Property land Details--------------------------------------------------------->
	           ...
	           <%--   	<div class="form-group">			
 					<apptags:input labelCode="property.propertyDeedNo" path="provisionalAssesmentMstDto.assDeedNo" maxlegnth="50"></apptags:input>
					</div> --%>
	...
	<c:if test="${command.getAssType() ne 'A' && command.getAssType() ne 'E'}">  
...
<%--                         	<th width="7%"><spring:message code="unitdetails.unittype"/></th> --%>
                        	...
                        	<th width="10%"><spring:message code="unitdetails.dateoffirstassessment"/></th>
...
<%-- 	                        <td class="text-center">${unitDetails.proAssdUnitType}</td> --%>
	                        ...
	                        <td class="text-center">${unitDetails.proAssdConstructionDate}</td>
	                         <td class="text-center">${unitDetails.firstAssesmentDate}</td>
...
<!-- 									</div> -->
      ...
      <h4 class="margin-top-10 margin-bottom-10 panel-title">
			<a data-toggle="collapse" href="#UnitDetail"><spring:message code="dataEntry.billDetail"/></a>
 			</h4>
		<div class="panel-collapse collapse in" id="UnitDetail">
		 ...
		 <label  class="col-sm-2  required-control ">
			...
			<div class="col-sm-4">
				<form:select id="financialYear" path="schduleId" class="form-control mandColorClass" data-rule-prefixValidation="true" onchange="getSchedule()" disabled="true">
					<form:option value="0">
...
<%-- 			<label class="col-sm-2 control-label required-control"><spring:message code="property.ConsolidatedTax" text="Consolidated Tax"/></label>
			<div class="col-sm-4">
		 <div class="input-group"> 
		<form:input path="provisionalAssesmentMstDto.assOldpropno" class="form-control" id="billDate" disabled="true"/>
		...
		<%-- <c:if test="${command.getAssType() eq 'DES'}">  --%>
		<c:choose>
		<c:when test="${not empty command.getBillMasList()}"> 
			<c:set var="d" value="0" scope="page" />
                <table id="taxdetailTable" class="table table-striped table-bordered appendableClass taxDetails">
                    <tbody>
                        <tr>
                        <th width="8%"><spring:message code="property.finYear" text="fin Year"/></th>
                 ...
                 <tr class="firstUnitRow ${masStatus.count%2==0? "trfirst": "trSecond"}" >
                    ...
                    <td width="150"> 
							...
							<td width="150"><form:input path="billMasList[${masStatus.count-1}].tbWtBillDet[${detStatus.count-1}].bdCsmp" type="text" class="form-control has2Decimal mandColorClass text-right" id="areear0" readonly="true"/></td>
              ...
              <td>${factorDetails.proAssfFactorValueDesc}</td>
  				...
  				<h4 class="margin-top-0 margin-bottom-10 panel-title">
						...
						<div class="panel-collapse collapse in" id="taxCollEmp">
							<div class="form-group">
							<apptags:input labelCode="dataEntry.taxColl.assign" path="provisionalAssesmentMstDto.taxCollEmpDesc" isDisabled="true"/>
...
<%--  <div class="form-group padding-top-10">
 		   ...
 		   <label for="file" class="col-sm-2 control-label"><spring:message
								...
								<div class="col-sm-4">
								 <apptags:formField fieldType="7" labelCode=""
																			hasId="true" fieldPath="checkList[0]"
    ...
    <c:if test="${not empty command.attachDocsList || command.modeType ne 'V'}">        	
  ...
  <div class="form-group">
		<div class="text-center padding-top-10">
		  <c:if test="${command.getAssType() ne 'E' || command.modeType eq 'A' }">  
		<c:set var="assType" value="${command.getAssType()}"></c:set>
              	<button type="button" class="btn btn-success btn-submit"
							onclick="saveDataEntryFrom(this,'${assType}')" id="submit">
							<spring:message code="property.dataEntry.save"/>
				</button>		
				</c:if>				
              	<button class="btn btn-blue-2" type="button"  onclick="editDataEntry(this)" id="edit">
										<spring:message code="property.Edit"/></button> 
							  <c:if test="${command.getAssType() eq 'E' && command.modeType eq 'V' }">  				
         <button class="btn btn-danger" type="button"  onclick="editDataEntry(this)" id="back">
