...
<ol class="breadcrumb">
	...
	<li class="active"><spring:message
			text="Search Document To Resubmit"
			code="cfc.resubmit.doc.upload.search" /></li>
</ol>
...
<div class="content">
	<div class="widget">
		...
		<div class="widget-content padding">
			...
			<form:form action="DocumentResubmission.html" name="frmMasterForm"
				id="frmMasterForm" class="form-horizontal">
				<jsp:include page="/jsp/tiles/validationerror.jsp" />
				<input type="hidden" id="resubmitDoc"
					value="<spring:message code="cfc.doc.resubmit"/>">
				<c:if test="${not command.resubmitedApplication }">
					...
					<div class="form-group">
						<label class="control-label col-sm-2 required-control"><spring:message
								code="cfc.application" text="Application Id" /></label>
						<div class="col-sm-4">
							<form:input path="applicationId" maxlength="16"
								cssClass="hasNumber form-control" />
						</div>
						<div class="col-sm-6">
							<a href="#" onclick="findAll(this)" class="btn btn-success"><i
								class="fa fa-search"></i> <spring:message code="bt.search" /></a>
						</div>
					</div>
				</c:if>
				...
				<c:if test="${not empty command.attachmentList}">
					<input type="hidden" value="${fn:length(command.attachmentList)}"
						id="docCount" />
					<h4>
						<spring:message code="cfc.resubmit.doc" />
					</h4>
								...
								<tr>
									...
									<td class="row1"><c:set var="links"
											...
											<apptags:filedownload filename="${singleDoc.lookUpCode}"
												filePath="${download}"
												dmsDocId="${singleDoc.extraStringField1}"
