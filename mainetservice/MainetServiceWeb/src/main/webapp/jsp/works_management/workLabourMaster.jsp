...
<div class="content">
	<div class="widget">
		<div class="widget-header">
			<h2>
				<spring:message code="works.add.labour" />
			</h2>
		...
		<div class="widget-content padding">
			...
			<form:form action="WorkEstimate.html" class="form-horizontal"
				...
				<form:hidden path="removeLabourFormById" id="removeLabourFormById" />
					...
					<table class="table table-bordered table-striped"
							...
							<tr>
								...
								<c:if test="${command.saveMode ne 'V'}">
									<th scope="col" width="10%"><a onclick='return false;'
										class="btn btn-blue-2 btn-sm addLabourFormDetails"> <i
						...
						<c:choose>
							<c:when test="${command.saveMode eq 'V'}">
								<c:forEach var="sorData" items="${command.addLabourList}"
									varStatus="status">

									<tr class="appendableClass">

										<td><form:hidden
												path="addLabourList[${d}].workEstemateId"
												id="workEstemateId${d}" /> <form:hidden
												path="addLabourList[${d}].workEstimFlag" id="rateTypeId${d}"
												value="A" /> <form:select path="" class="form-control"
												id="selectType${d}" onchange="rateMachinaryData(${d});"
												disabled="true">
												<form:option value="">
										...
										<td><form:select path="addLabourList[${d}].gRateMastId"
												...
												<c:forEach items="${command.addLabourList[d].rateList}"
													var="rateList">
										...
										<td><form:input
												path="addLabourList[${d}].workEstimQuantity"
										...
										<td><form:input
												path="addLabourList[${d}].workEstimAmount"
							...
							<c:when test="${fn:length(command.addLabourList) > 0}">
								<c:forEach var="sorData" items="${command.addLabourList}"
									varStatus="status">
									...
									<tr class="appendableClass">
										...
										<td class="text-center"><a href='#'
											...
											class='btn btn-danger btn-sm deleteLabourFormDetails'><i
							...
							<c:otherwise>
								<tr class="appendableClass">
									...
									<td><form:hidden path="addLabourList[${d}].workEstemateId"
											id="workEstemateId${d}" /> <form:hidden
											path="addLabourList[${d}].workEstimFlag" id="rateTypeId${d}"
											value="A" /> <form:select path="" class="form-control"
											id="selectType${d}" onchange="rateMachinaryData(${d});">
									...
									<td><form:input
											...
											onchange="getAmountFormatInDynamic((this),'workEstimQuantity')"
											onkeyup="calculateTotal();" placeholder="000.00" /></td>
									<td><form:input path="addLabourList[${d}].workEstimAmount"
											class="form-control text-right allamount"
											id="workEstimAmount${d}" readonly="true"
