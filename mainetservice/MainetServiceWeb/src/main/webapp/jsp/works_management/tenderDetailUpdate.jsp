			...
			<form:form action="TenderInitiation.html" class="form-horizontal"
				...
				<div class="form-group">
					...
					<label for="select-start-date" class="col-sm-2 control-label"><spring:message
							code="tender.total.probable.amount.of.contract" text="" /> </label>
					<div class="col-sm-4">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<form:input path="initiationDto.tenderNo"
							...
							readonly="${command.initiationDto.loaGenerated}" />
					</div>
					<label class="col-sm-2 control-label"><spring:message
					...
					<div class="col-sm-4">
						<div class="input-group">
							<form:input path="initiationDto.workDto[0].tndSubmitDate"
								...
								value="" readonly="true"
								disabled="${command.mode eq 'V' || command.initiationDto.loaGenerated}" />
				...
				<div class="form-group">
					<label class="col-sm-2 control-label" for="tndPGRate"><spring:message
							code="tender.securityRateDeposite"
							text="Initial Security Deposit /Performance Security Percentage rate" /></label>

					<div class="col-sm-4">
						<form:input path="initiationDto.workDto[0].tndPGRate"
							id="tndPGRate" class="form-control preventSpace hasAmount"
							maxlength="50" readonly="${command.initiationDto.loaGenerated}"
					...
					<label class="col-sm-2 control-label" for="tndPGAmount"><spring:message
							code="tender.securityAmount"
							text="Initial Security Deposit /Performance Security Amount" /></label>
					<div class="col-sm-4">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<form:select path="initiationDto.workDto[0].tndAuthDesgid"
							...
							disabled="${command.initiationDto.loaGenerated}">
							<form:option value="0">
					...
					<label class="col-sm-2 control-label "><spring:message
							code="tender.depositType" text="Deposit Type" /></label>
					<c:set var="baseLookupCode" value="DTY" />
					<apptags:lookupField
						items="${command.getLevelData(baseLookupCode)}"
						path="initiationDto.workDto[0].tndPGTypeId"
				...
				<div class="form-group">
					...
					<label class="col-sm-2 control-label "><spring:message
							code="tender.paymentMode" text="Payment Mode" /></label>
					<c:set var="baseLookupCode" value="PAY" />
					<apptags:lookupField
						items="${command.getLevelData(baseLookupCode)}"
						path="initiationDto.workDto[0].tndPGModeId"
						cssClass="form-control required-control chosen-select-no-results"
						selectOptionLabelCode="selectdropdown" hasId="true"
						disabled="${command.initiationDto.loaGenerated}" />
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<label for="including" class="radio-inline mandColorClass"><form:radiobutton
								...
								disabled="${command.initiationDto.loaGenerated}" /> <spring:message
						...
						<thead>
							<tr>
								...
								<th width="10%"><spring:message code=''
										text="Percentage-Is" /><span class="mand">*</span></th>
								...
								<!-- Removed Field AS per SUDA UAT
								perspective 27/10/2018  -->
								<%-- <th width="10%"><spring:message code=''
										text="Stamp Paper Fee" /></th> --%>
								<th width="9%"><spring:message
										code='wms.NoOfDaysForAggrement'
										text="No. Of Days For Agreement" /><span class="mand">*</span></th>
								<c:if test="${command.loaMode eq null}">
									<th width="6%"><spring:message code='wms.LOAGenerate'
											text="LOA Generate" /></th>
								</c:if>
								<c:if test="${command.loaMode eq 'P'}">
									<th width="10%"><spring:message
											code='works.management.print.loa' text="Print LOA" /></th>
								</c:if>
							</tr>
						</thead>
						<tbody>
							<c:forEach items="${command.initiationDto.workDto}" var="workDto">
								...
								<tr class="tenderWorkUpdate">
									...
									<td class="tenderTypes"><form:select
											...
											disabled="${workDto.tenderInitiated}">
											<form:option value="">
												<spring:message code='work.management.select' />
											</form:option>
									...
									<td><form:select
											...
											data-rule-required="true"
											disabled="${workDto.tenderTypeCode ne 'PER' || command.mode eq 'V'? true : false}">
											<form:option value="">
												<spring:message code='work.management.select' />
											</form:option>
									...
									<td><form:input
											...
											readonly="${workDto.tenderInitiated}" /></td>
									...
									<td><form:select
											...
											id="vendorId${c}" disabled="${workDto.tenderInitiated}"
											data-rule-required="true">
											<form:option value="">
												<spring:message code='work.management.select' />
											</form:option>
									...
									<c:if test="${command.loaMode eq null}">
										<td align="center"><form:checkbox
												path="initiationDto.workDto[${c}].tenderInitiated"
												id="tenderInitiated${c}" class="checkbox-inline"
												style="margin-top:8px"
												onclick="return ${! workDto.tenderInitiated}"
												checked="${workDto.workTypeCode eq 'L'? 'checked' : '' }" />
										</td>
									</c:if>

									<c:if test="${command.loaMode eq 'P'}">
										<td><center> <button type="button" class="btn btn-warning btn-sm"
												name="" value="Print"
												onclick="printLoaByTenderDetislId(${workDto.tndWId})"
												id="button-Cancel"
												title="<spring:message code="works.management.print.loa" text="LOA"></spring:message>">
				...
				<div class="text-center clear padding-10">
					<c:if test="${command.loaMode ne 'P'}">
						<c:if test="${command.tenderUpdated ne 'Y'}">
							<button class="btn btn-success add"
								onclick="saveupdatedTender(this);" type="button">
