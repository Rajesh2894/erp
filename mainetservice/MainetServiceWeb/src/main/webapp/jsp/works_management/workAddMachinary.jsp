...
<div class="content">
	<div class="widget">
		<div class="widget-header">
			<h2>
				<spring:message code="works.add.machinery" />
			</h2>
		...
		<div class="widget-content padding">
			...
			<form:form action="WorkEstimate.html" class="form-horizontal"
				...
				<form:hidden path="removeMachineryById" id="removeMachineryById" />
					...
					<table class="table table-bordered table-striped"
						...
						<thead>
							<tr>
								...
								<th width="10%"><spring:message
										code="work.estimate.select.from" text="Select From" /><span
									class="mand">*</span></th>
								<th width="20%"><spring:message
										code="work.estimate.from.master" text="From Master" /><span
									class="mand">*</span></th>
								<th width="10%"><spring:message code="work.estimate.rate"
										text="Rate" /><span class="mand">*</span></th>
								<th width="10%"><spring:message code="work.management.unit"
								...
								<c:if test="${command.saveMode ne 'V'}">
									<th scope="col" width="10%"><a onclick='return false;'
										class="btn btn-blue-2 btn-sm addMachineryDetails"> <i
						...
						<c:choose>
							<c:when test="${command.saveMode eq 'V'}">
								<c:forEach var="sorData" items="${command.addMachinaryList}"
									varStatus="status">

									<tr class="appendableClass">

										<td><form:hidden
												path="addMachinaryList[${d}].workEstemateId"
												id="workEstemateId${d}" /> <form:hidden
												path="addMachinaryList[${d}].workEstimFlag"
												id="rateTypeId${d}" value="C" /> <form:select path=""
												class="form-control" id="selectType${d}"
												onchange="rateMachinaryData(${d});" disabled="true">
												<form:option value="">
										...
										<td><form:select
												path="addMachinaryList[${d}].gRateMastId"
												...
												<c:forEach items="${command.addMachinaryList[d].rateList}"
													var="rateList">
										...
										<td><form:input id="rate${d}" path=""
												class=" form-control text-right" maxlength="5"
												readonly="true" placeholder="00.00" /></td>
										...
										<td><form:input
												path="addMachinaryList[${d}].workEstimQuantity"
												class="form-control text-right" disabled="true"
												id="workEstimQuantity${d}" placeholder="000.00"
										...
										<td><form:input
												path="addMachinaryList[${d}].workEstimAmount"
							...
							<c:when test="${fn:length(command.addMachinaryList) > 0}">
								<c:forEach var="sorData" items="${command.addMachinaryList}"
									varStatus="status">
									...
									<tr class="appendableClass">
										...
										<td><form:input id="rate${d}" path=""
												...
												maxlength="5" placeholder="00.00" /></td>
										...
										<td><form:input
												...
												onkeypress="return hasAmount(event, this, 3, 2)"
												onchange="getAmountFormatInDynamic((this),'workEstimQuantity')"
												onkeyup="calculateTotal();" placeholder="000.00" /></td>
										...
										<td class="text-center"><a href='#'
											...
											class='btn btn-danger btn-sm deleteMachineryDetails'><i
							...
							<c:otherwise>
								<tr class="appendableClass">
									...
									<td><form:select path="addMachinaryList[${d}].gRateMastId"
											class="form-control" id="gRateMastId${d}"
											onchange="getRateUnitBySorId(${d});">
											<form:option value="">
												<spring:message code="holidaymaster.select" />
