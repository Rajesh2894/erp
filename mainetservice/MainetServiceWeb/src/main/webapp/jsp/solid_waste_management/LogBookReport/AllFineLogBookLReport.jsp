	...
	<div class="content">
		<div class="widget">
			<div class="widget-header">
				<spring:message code="" text="All Fine Log Book Report" />
			</div>
					...
					<div id="receipt">
						...
						<div class="col-xs-8 col-xs-8  text-center">
							<h2 class="text-large margin-bottom-0 margin-top-0 text-bold">
								...
								District - ${command.fineChargeCollectionDTO.districtName} <br>Fine
								Information For Month -
								${command.fineChargeCollectionDTO.monthName} <br>
								<spring:message code="" text="All Fine Charge Report" />
							</h2>
						</div>
						<div class="col-xs-2">
							<p>
						...
						<div id="export-excel">
							<table class="table table-bordered table-condensed">
								...
								<tbody>
									<c:forEach
										items="${command.fineChargeList.fineChargeCollectionList}"
										var="data" varStatus="index">
										<tr>
										    <td class="text-center">${index.count}</td>
											<td class="text-center">${data.fchEntrydate}</td>
											<td>${data.fchName} </td>
											<td class="text-right">${data.fchMobno}</td>
											<td>${data.address}</td>
											<td>${data.wardEng}</td>
											<td>${data.chargeDescEng}</td>
											<td></td>
											<td class="text-right">${data.reiceptNo}</td>
											<td class="text-right">${data.chargeAmount} </td>
											<td>${data.chargeDescEng}</td>
										</tr>
									</c:forEach>
								</tbody>
