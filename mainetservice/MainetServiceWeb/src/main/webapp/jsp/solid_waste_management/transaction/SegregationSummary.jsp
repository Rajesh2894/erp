		...
		<div class="widget-content padding">
			...
			<form:form action="Segregation.html" commandName="command"
				...
				<div class="form-group">
					<label class="col-sm-2 control-label " for="Desposal Site"><spring:message
							code="swm.dsplsite" /></label>
					<div class="col-sm-4">
						<form:select path="segregationDto.deId" class="form-control  "
				...
				<div class="form-group">
					<apptags:date fieldclass="fromDateClass" labelCode="swm.fromDate"
						datePath="segregationDto.fromDate" cssClass="fromDate" >
					</apptags:date>
					<apptags:date fieldclass="toDateClass" labelCode="swm.toDate"
						datePath="segregationDto.toDate"
						cssClass="toDate" >
					</apptags:date>
				</div>
				<!-- Start button -->
				<div class="text-center clear padding-10">
					<button type="button" class="btn btn-blue-2 search"
						onclick="searchSegregation('Segregation.html','SearchSegregation')">
						<i class="fa fa-search"></i>
						<spring:message code="solid.waste.search" text="Search" />
					</button>
					<%-- Defect #152968 --%>
					<button type="button"
						onclick="javascript:openRelatedForm('Segregation.html');"
				...
				<div class="table-responsive">
					<div class="table-responsive margin-top-10">
						<table class="table table-striped table-condensed table-bordered"
							...
							<thead>
								<tr>
									<th width="8%"><spring:message code="vehicle.master.vehicle.srno"
											text="Sr. No." /></th>
									<th><spring:message code="swm.date" text="Date" /></th>
									<th><spring:message code="swm.dsplsite" text="Waste Segregation Center" /></th>							
									<th><spring:message code="swm.wastetype" text="Waste Type" /></th>
									<th><spring:message code="swm.ttlvolume"
											text="Total Volume" /></th>
									<th width="10%"><spring:message code="swm.action"
											text="Action" /></th>
								</tr>
							</thead>
							<tbody>
								<c:forEach items="${command.tbSwWastesegDets}" var="seg"
									varStatus="loop">
									<tr>
										<td align="center" width="8%">${loop.count}</td>
										<td align="center"><fmt:formatDate pattern="dd/MM/yyyy"
												value="${seg.grDate}" /></td>
										<td align="center">${seg.dName}</td>							
										<td align="center">${command.getHierarchicalLookUpObject(seg.tbSwWastesegDets[0].codWast1).getLookUpDesc()}</td>
										<td align="right">${seg.totalVol} &nbsp;<spring:message
												code="swm.kilo" text="Kgs" /></td>
										<td class="text-center" width="10%">
											<button type="button" class="btn btn-blue-2 btn-sm"
												title="View "
												onclick="modifySegregation(${seg.grId},'Segregation.html','ViewSegregation','V')">
												<i class="fa fa-eye"></i>
											</button>
											<button type="button" class="btn btn-warning btn-sm"
												title="Edit "
												onclick="modifySegregation(${seg.grId},'Segregation.html','EditSegregation','E')">
