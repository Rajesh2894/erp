...
@media print {
	...
	.table-responsive {
		...
		border: none;
			...
			<form:form id="fpoAssesementApproval"
				action="FPOAssessmentApproval.html" method="post"
				...
				<div class="table-responsive">
					<table class="table table-bordered table-striped"
						id="fpoAssessEntTable" width="100%">
						<thead>
							<tr>
							...
							<tr>
								<th width="5%"><spring:message code="sfac.key.parameter"
										text="Key Parameter" /></th>
								<th width="5%"><spring:message code="sfac.weightage"
										text="Weightage" /></th>
								<th>
										...
										<thead>
											<tr>
												<th width="10%"><spring:message
														code="sfac.sub.parameter" text="Sub Parameter" /></th>
												<th>
													<table class="inner-table-2">
														<thead>
															<tr>
																...
																<th width="5%"><spring:message
																		code="sfac.sub.weightage" text="Sub Weightage" /></th>
																<th width="8%" class="required-control"><spring:message
																		code="" text="Marks Awarded" /></th>
																<th width="5%" class="required-control"><spring:message
																		code="sfac.ass.score" text="Score %" /></th>
																<th width="8%" class="required-control"><spring:message
																		code="sfa.remark" text="Remark" /></th>
															</tr>
														</thead>
													</table>
												</th>
						...
						<tbody>
							...
							<c:forEach var="assMastDto" items="${command.keyParamDtoList}"
								varStatus="status">
								<tr>
									<td class="text-center vertical-align-middle"><form:hidden
											path="keyParamDtoList[${d}].keyParameter"
											id="keyParameter${d}" class="keyParameter" />${assMastDto.keyParameterDesc}</td>
									...
									<td><c:set var="e" value="0" scope="page"></c:set>
										<table class="inner-table">
											<c:forEach var="detDto"
												items="${assMastDto.fpoSubParamDtoList}" varStatus="status">
												<tr>
													<td>${detDto.subParameterDesc}</td>
													<td><c:set var="f" value="0" scope="page"></c:set> <c:forEach
															var="subDto" items="${detDto.fpoSubParamDetailDtoList}">

															<table class="inner-table-2">
																<tr>
																	<td>${subDto.conditionDesc}</td>
																	<td>${subDto.subWeightage}</td>
																	<c:set var="w" value="${w + subDto.subWeightage}"
																		scope="page" />
																	<td><form:input class="form-control hasNumber"
																			disabled="true"
																			onblur="calculateScore('${d}${e}${f}');"
																			maxlength="3" id="marksAwarded${d}${e}${f}"
																			path="keyParamDtoList[${d}].fpoSubParamDtoList[${e}].fpoSubParamDetailDtoList[${f}].marksAwarded" />
																	</td>
																	<td><form:input class="form-control hasNumber"
																			disabled="true"
																			onblur="calculateTotal('${d}${e}${f}');"
																			maxlength="3" id="score${d}${e}${f}"
																			path="keyParamDtoList[${d}].fpoSubParamDtoList[${e}].fpoSubParamDetailDtoList[${f}].score" />
																	</td>
																	<td><form:input class="form-control"
																			id="remark${d}${e}${f}" disabled="true"
																			path="keyParamDtoList[${d}].fpoSubParamDtoList[${e}].fpoSubParamDetailDtoList[${f}].remark" />
																	</td>
																</tr>
															</table>
												...
												<table class="inner-table-2">
													<tr>
														...
														<th><form:input class="form-control" readonly="true"
																id="totalMarksAwarded"
																path="assementMasterDto.totalMarksAwarded" /></th>
														<th><form:input class="form-control" readonly="true"
				...
				<div class="text-center margin-top-20">
					...
					<button type="button" class="btn btn-danger hidden-print"
						onclick="javascript:openRelatedForm('FPOAssessmentEntry.html');">
