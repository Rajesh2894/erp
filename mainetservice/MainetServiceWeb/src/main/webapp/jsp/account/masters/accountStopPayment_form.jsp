...
<c:if test="${stopPayment.hasError =='true'}">
...
<div class="">
	<form:form class="form-horizontal" modelAttribute="stopPayment"
		method="POST" action="AccountStopPayment.html">
						...
						<div class="panel-heading">
							<h4 type="h4" class="panel-title table" id="">
								<a data-toggle="collapse" class="collapsed"
									data-parent="#accordion_single_collapse" href="#a2">Stop
									Payment Details : ${categoryTypeDesc} : ${bankTypeDesc} </a>
							</h4>
						</div>
							...
							<table class="table table-bordered table-striped main1"
								...
								<thead>
									<tr class="bankRecTable">
										...
										<th>Amount</th>
										<th>Stop Payment Date <span class="mand">*</span></th>
										<th>Stop Payment Reason <span class="mand">*</span></th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${stopPayment.stopPaymentDto}"
										...
										<tr class="chequeTr">
											<form:hidden path="stopPaymentDto[${count}].paymentMode"
												id="transactionMode${count}" />
											<form:hidden path="stopPaymentDto[${count}].serchType"
												id="serchType${count}" />
											<form:hidden path="stopPaymentDto[${count}].depositSlipId"
												id="depositSlipId${count}" />
											<form:hidden path="stopPaymentDto[${count}].id"
												id="id${count}" />
											<form:hidden path="stopPaymentDto[${count}].previousDate"
												id="previousDate${count}" />
											<form:hidden path="stopPaymentDto[${count}].chequebookDetid"
												id="chequebookDetid${count}" />

											<td><form:input
													path="stopPaymentDto[${count}].paymentDate"
													id="transactionDate${count}" class=" form-control "
													readonly="true" name="text-01" /></td>
											<td><form:input
													path="stopPaymentDto[${count}].paymentNo"
													id="transactionNo${count}" class=" form-control "
													readonly="true" name="text-01" /></td>
											<td><form:input
													path="stopPaymentDto[${count}].transMode"
											...
											<%-- <td><form:input
													...
													readonly="true" name="text-01" /></td> --%>
											<td><form:input
													path="stopPaymentDto[${count}].chequeddno"
													id="transactionNo${count}" class=" form-control "
													readonly="true" name="text-01" /></td>

											<td><form:input
													path="stopPaymentDto[${count}].chequedddate"
													id="transactionNo${count}" class=" form-control "
													readonly="true" name="text-01" /></td>

											<td class="text-right"><form:input type="text"
													cssClass="form-control text-right"
													path="stopPaymentDto[${count}].amount" id="amount${count}"
													readonly="true" /></td>

											<td><form:input
													path="stopPaymentDto[${count}].stopPaymentDate"
													id="date${count}"
													cssClass="form-control mandColorClass datepicker"
													onchange="changeValidIssueaneDate(${count})" maxlength="10" /></td>

											<td><form:input
													path="stopPaymentDto[${count}].stopPaymentReason"
													id="stopPaymentReason${count}"
													cssClass="form-control mandColorClass" maxlength="500" /></td>
										</tr>
									</c:forEach>
								</tbody>
								<tfoot>
									<c:if test="${stopPayment.balanceAsUlb ne '0.00' }">
										<tr>
		...
		<div class="text-center padding-top-10">
			...
			<%-- </c:if> --%>
			<spring:url var="cancelButtonURL" value="AccountStopPayment.html" />
