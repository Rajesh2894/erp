...
<script type="text/javascript" src="js/birthAndDeath/deathRegisterFemale.js"></script> 
...
<div class="content">
	<div class="widget">
		<div class="widget-header">
			<h2>
				<spring:message code="DeathRegisterDTO.form" text="Death Register Only For Female" />
...
<button type="button" class="close" onclick="closeOutErrBox()" aria-label="Close"><span aria-hidden="true">&times;</span></button>
		...
		<form:form action="DeathRegister.html" id="frmbirthregister" method="POST" commandName="command" class="form-horizontal form">
		<h4>
		...
		<apptags:date fieldclass="datepicker"
				labelCode="DeathRegisterDTO.periodFrom"
				datePath="deathRegisterDTO.periodFrom" isMandatory="true"
				cssClass="custDate mandColorClass date">
			</apptags:date>	
			
			<apptags:date fieldclass="datepicker"	
			labelCode="DeathRegisterDTO.periodTo"
			datePath="deathRegisterDTO.periodTo" isMandatory="true"
			cssClass="custDate mandColorClass date">
			</apptags:date>
			</div>
			
			<div class="form-group">
						<label class="col-sm-2 control-label required-control" for="user"><spring:message code="DeathRegisterDTO.user" text="User" /></label>
					 
					<div class="col-sm-4">
						<form:select id="user" path="deathRegisterDTO.user" cssClass="chosen-select-no-results"  data-rule-required="true" multiple="true" >
							
							<form:option value="0" label="All"/>
							<c:forEach items="${bndDeptEmployee}" var="user">
							  
								<form:option value="${user.empId}">${user.fullName}</form:option>
							</c:forEach>
						</form:select>
					</div> 
				</div>
