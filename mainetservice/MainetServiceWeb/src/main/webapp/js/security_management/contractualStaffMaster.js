...
function checkDOBAge() {
	$('#dob').on('change', function(){
		var errorList = [];
		var getDateFromDatePicker = $(this).datepicker('getDate');
		var day = getDateFromDatePicker.getDate();
		var month = getDateFromDatePicker.getMonth() + 1;
		var year = getDateFromDatePicker.getFullYear();
		var age = 18;
		var setDate = new Date(year + age, month - 1, day);
		var currdate = new Date();
		if (currdate <= setDate) {
			errorList.push(getLocalMessage("ContractualStaffMasterDTO.validation.age"));
					...
					result.push([
								...
								result.push([
									...
									'<div class="text-center">'+
											...
											'<a href="#" class="fa fa-times-circle fa-2x red" title="Active"></a>'+
											'</c:when>'+
											'</c:choose>'+'</div>',
									'<div class="text-center">'
											+ '<button type="button"  class="btn btn-blue-2 btn-sm margin-right-5"  onclick="getContractualStaffMaster(\''
...
function validateContractualStaffMaster(errorList,mode) {
	...
	var contStsffId = $("#contStsffId").val();
	var sex = $("#sex").val();
...
//		errorList.push(getLocalMessage("ContractualStaffMasterDTO.validation.cpdShiftId"));
	...
	if(contStaffMob != "" && contStaffMob != null){
		var URL="ContractualStaffMaster.html?checkMobileNumber";
		var requestData='contStaffMob='+contStaffMob+'&contStsffId='+contStsffId;;
		
		var resData = __doAjaxRequest(URL, 'POST', requestData, false);
		if(resData==true){
			errorList.push(getLocalMessage("ContractualStaffMasterDTO.Validation.duplicatemobile"));
			}
	}
...
function getEmployeeList(){
	...
	$('#empId').prop('disabled', false).trigger("chosen:updated"); //Defect #150836
