...
function resetCRForm() {
	...
	$('input[type=text]').val('');
	$(".alert-danger").hide();
	$("#CollectionRegister").validate().resetForm();
}
function saveCRForm(obj) {
	...
	if (errorList.length > 0) {
		...
		var URL = 'waterCollectionRegister.html?GetCollectionDetails';
...
function saveXLSXForm(obj) {
	...
	if (errorList.length > 0) {
		...
		var URL = 'waterCollectionRegister.html?GetDetailXLSX';
		var returnData = __doAjaxRequest(URL, 'POST', requestData, false);
		if (returnData == "f") {
			errorList.push(getLocalMessage("property.report.finYear.date"));
			displayErrorsOnPage(errorList);
