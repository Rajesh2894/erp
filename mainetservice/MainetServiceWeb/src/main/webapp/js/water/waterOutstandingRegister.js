$(document).ready(function() {
	...
	/*
	 * var radios = $('input:radio[Value]'); if (radios.is(':checked') ===
	 * false) { radios.filter('[value]').prop('checked', true); }
	 */
...
function saveORForm(obj) {
	...
	if (ReportType == 'D' || ReportType == 'S') {
		...
		if (csFromdt == 0) {
			errorList.push(getLocalMessage('water.report.validation.fromdate'));
		}
	}
	...
	if (errorList.length > 0) {
		...
		var URL = 'waterOutstandingRegister.html?GetOutstandingDetails';
...
$("#resetform").on("click", function() {
	window.location.reload("#OutstandingRegister")
});
