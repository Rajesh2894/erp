...
$(document).ready(function() {
	...
	$('#calculation tr').each(function(i) {		
		var totalAmount = $("#depAmount" + i).text();
		if (totalAmount != 0) {
			grandTotal += isNaN(Number(totalAmount)) ? 0 : Number(totalAmount);
		}
	});
	
	$("#totalDeposit").text(grandTotal.toFixed(2));
...
function viewDepositRegisterReport() {
	...
	if (errorList.length > 0) {
		...
		var ajaxResponse = doAjaxLoading(
				'DepositRefundLibilityRegisterReport.html?viewDepositRegisterReport', requestData,
				'html');
...
function backReportForm() {		
		...
		$("#postMethodForm").prop('action', 'DepositRefundLibilityRegisterReport.html');
