	...
	kendo.drawing
			...
			.then(
					function(group) {
						kendo.drawing.pdf
								.saveAs(group,
										"NAGAR NIGAM / NAGAR PALIKA PARISHAD / NAGAR PANCHAYAT.pdf")
					});
}

function ExportPdfPQR() {
	kendo.drawing.drawDOM("#myCanvas", {
		...
		kendo.drawing.pdf.saveAs(group, "Pre_Qualification_Document.pdf")
	});
}

function ExportPdfFormA() {
	kendo.drawing.drawDOM("#myCanvas", {
		forcePageBreak : ".page-break",
		paperSize : "A4",
		// margin: { top: "0cm", bottom: "0cm" },
		...
		kendo.drawing.pdf.saveAs(group, "FORM A.pdf")
	});
}
...
function exportHTML() {
	...
	fileDownload.download = 'NAGAR NIGAM / NAGAR PALIKA PARISHAD / NAGAR PANCHAYAT.doc';
	fileDownload.click();
	document.body.removeChild(fileDownload);
}

function exportHTMLPQR() {
	var header = "<html xmlns:o='urn:schemas-microsoft-com:office:office' "
			+ "xmlns:w='urn:schemas-microsoft-com:office:word' "
			+ "xmlns='http://www.w3.org/TR/REC-html40'>"
			+ "<head><meta charset='utf-8'><title>Export HTML to Word Document with JavaScript</title></head><body>";
	...
	document.body.appendChild(fileDownload);
	fileDownload.href = source;
	fileDownload.download = 'Pre_Qualification_Document.doc';
	fileDownload.click();
	document.body.removeChild(fileDownload);
}

function exportHTMLFormA() {
	var header = "<html xmlns:o='urn:schemas-microsoft-com:office:office' "
			+ "xmlns:w='urn:schemas-microsoft-com:office:word' "
			+ "xmlns='http://www.w3.org/TR/REC-html40'>"
			+ "<head><meta charset='utf-8'><title>Export HTML to Word Document with JavaScript</title></head><body>";
	...
	document.body.appendChild(fileDownload);
	fileDownload.href = source;
	fileDownload.download = 'FORM_A.doc';
	fileDownload.click();
	document.body.removeChild(fileDownload);
}
