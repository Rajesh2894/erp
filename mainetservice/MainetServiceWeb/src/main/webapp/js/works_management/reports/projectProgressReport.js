...
function viewProjectProgressReport() {
	...
	if (errorList.length > 0) {
		...
		var ajaxResponse = doAjaxLoading(
				'ProjectProgressReport.html?getProjectProgressReportData', requestData,
				'html');
...
function PrintDiv(title) {
	...
	printWindow.document.write('<script>$(window).on("load",function() {$(".table-pagination, .remove-btn, .paging-nav, tfoot").remove(); $(".table thead tr th").removeClass("tablesorter-headerDesc tablesorter-headerAsc tablesorter-header"); $(".table tr").removeAttr("style");});</script>')
