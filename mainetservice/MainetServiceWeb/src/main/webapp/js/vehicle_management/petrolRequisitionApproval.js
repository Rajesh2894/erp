...
function savePetrolReqApprovalData(element) {
	...
	if($("#petrolRegRemark").val()==""){
		errorList.push(getLocalMessage("PetrolRequisition.remarkValidation"));
	 ...
	 if(object.error != null && object.error != 0  ){	
		...
		if(object.PetrolWfStatus =='REJECTED'){
			 showBoxForApproval(getLocalMessage('Petrol.Requisition.reject'));
		 }else if(object.PetrolWfStatus =='SEND_BACK'){
			 showBoxForApproval(getLocalMessage('Petrol.Requisition.sendBack'));
		 }else if(object.PetrolWfStatus =='FORWARD_TO'){
			 showBoxForApproval(getLocalMessage('Petrol.Requisition.sendBack'));
		 }else{
			 showBoxForApproval(getLocalMessage("PetrolRequisition.authSuccess"));
		 }
	 } 
   }
}
...
function savePetrolReqApprovalData12345(element) {
	...
	if($("#petrolRegRemark").val()==""){
	 ...
	 if(object.error != null && object.error != 0  ){	
		...
		if(object.PetrolWfStatus =='REJECTED'){
			 showBoxForApproval(getLocalMessage('Petrol.Requisition.rejection'));
		 }else if(object.PetrolWfStatus =='SEND_BACK'){
			 showBoxForApproval(getLocalMessage('Petrol.Requisition.sendBack'));
		 }else if(object.PetrolWfStatus =='FORWARD_TO'){
			 showBoxForApproval(getLocalMessage('Petrol.Requisition.sendBack'));
...
function showBoxForApproval(succesMessage){
	...
	var message = '';
	var Proceed = getLocalMessage('vehicle.proceed');// 'Proceed';
	var no = 'No';	
