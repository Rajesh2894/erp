...
$(document)
		.ready(
				function() {
					...
					$("#wrStartTime")
							.val($("#wrStartTime").val().slice(10, 16));
					$("#wrEndTime").val($("#wrEndTime").val().slice(10, 16));

					var selectedType = $("#wrWeekType").find("option:selected")
							.attr('code');
					...
					var valArrOdd = $("#wrOddWorkWeek").val().split(",");

					var arrayLengthOdd = valArrOdd.length;
					for (var i = 0; i < arrayLengthOdd; i++) {
						$(".oddWorkWeek").find(
								":checkbox[value='" + valArrOdd[i] + "']")
								.attr("checked", "checked");
						$(".oddWorkWeek option[value='" + valArrOdd[i] + "']")
								.attr("selected", 1);
					}

					var valArrEven = $("#wrEvenWorkWeek").val().split(",");

					var arrayLengthEven = valArrEven.length;
					for (var i = 0; i < arrayLengthEven; i++) {
						$(".evenWorkWeek").find(
								":checkbox[value='" + valArrEven[i] + "']")
								.attr("checked", "checked");
						$(".evenWorkWeek option[value='" + valArrEven[i] + "']")
								.attr("selected", 1);
					}
					...
					var valArrNorm = $("#wrWorkWeek").val().split(",");

					var arrayLengthNorm = valArrNorm.length;
					for (var i = 0; i < arrayLengthNorm; i++) {
						$(".workWeek").find(
								":checkbox[value='" + valArrNorm[i] + "']")
								.attr("checked", "checked");
						$(".workWeek option[value='" + valArrNorm[i] + "']")
								.attr("selected", 1);
					}
				});
		...
		function() {
			...
			$(document).ready(function() {
				...
				$(".workWeek").click(function() {
					var selectedLanguage = new Array();
					$('input[name="selector3"]:checked').each(function() {
						selectedLanguage.push(this.value);
					});
...
function saveHolidayDetails(element) {
	...
	if ($("#hoYearStartDate").val() == "")
		errorList.push(getLocalMessage("master.startDate.not.null"));

	var tbody = $("#calTable tbody");

	if (tbody.children().length == 0) {
		errorList.push(getLocalMessage("master.holidayDate.not.null"));
	}
