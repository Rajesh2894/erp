...
function DayMonthWiseReportPrint(formUrl, actionParam) {
	var errorList = [];
...
function validateReportList(errorList) {
	...
	var fromDate = $('#fromDate').val();
	if (fromDate == "0" || fromDate == "" || dname == null) {
		errorList.push(getLocalMessage('swm.fromdate.selected'));
	}
	var toDate = $('#toDate').val();
	if (toDate == "0" || toDate == "" || dname == null) {
...
function back() {
	...
	$("#postMethodForm").prop('action', '');
	$("#postMethodForm").prop('action', 'DayMonthWiseDumping.html');
