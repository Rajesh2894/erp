function SweepingReportPrint(formUrl) {
	
	var errorList = [];
	var beatId  = $('#beatId').val();
	var wasteTypeId= $('#wasteTypeId').val();
	var monthNo = $('#monthNo option:selected').attr('code');
	var beatName= $('#beatId option:selected').text();
	var monthName= $('#monthNo option:selected').text();
	if(monthNo=="0" || monthNo==null){
		errorList.push("Month Name Must be Selected.");
	}
	if (beatId == "0" || beatId == null ) {
		errorList.push("Beat Name Must be Selected");
	}
	...
	var ajaxResponse = doAjaxLoading(formUrl + '?' + "sweeping", data, 'html',divName);
...
function back() {
	...
	$("#postMethodForm").prop('action', '');
	$("#postMethodForm").prop('action', 'VehicleLogBookMainPageReportPrint');
