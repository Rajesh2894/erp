...
function backVehicleMaintenanceMasterForm() {  //backVehicleMaintenanceMasterForm()
	...
	$("#postMethodForm").prop('action', 'VehicleMaintenanceMaster.html');
...
function validateFormData(errorList){
		...
		if (vemReceiptdate == "" || vemReceiptdate == null || vemReceiptdate == 'undefined') {
			errorList.push(getLocalMessage('vehicle.maintenance.receiptDate')+getLocalMessage('solid.waste.validation.empty'));
		}
		if (vemReceiptno == "" || vemReceiptno == null || vemReceiptno == 'undefined') {
			errorList.push(getLocalMessage('vehicle.maintenance.receiptno')+getLocalMessage('solid.waste.validation.empty'));
		}
		if (veVetype == "" || veVetype == null || veVetype == 'undefined' || veVetype == "0") {
			errorList.push(getLocalMessage('vehicle.maintenance.master.type')+getLocalMessage('solid.waste.validation.empty'));
		}	
		if (vemDowntime == "" || vemDowntime == null || vemDowntime == 'undefined') {
			errorList.push(getLocalMessage('vehicle.maintenance.master.estimatedDowntime')+getLocalMessage('solid.waste.validation.empty'));
		}	
		if (vemDowntimeunit == "" || vemDowntimeunit == null || vemDowntimeunit == 'undefined' || vemDowntimeunit == "0") {
			errorList.push(getLocalMessage('vehicle.maintenance.downtime.unit')+getLocalMessage('solid.waste.validation.select'));
		}	
		return errorList;
	}
...
function saveVehicleMaintenanceMasterForm(element){
	...
	if (errorList.length > 0) {
		...
		$("#errorDiv").hide();
		return saveOrUpdateForm(element,getLocalMessage('vehicle.maintainance.master.add.success'), 'VehicleMaintenanceMaster.html', 'saveform');
	}
}
...
function proceedForDelete(vemeId) {
	...
	var divName = '.content-page';
	var ajaxResponse = doAjaxLoading('VehicleMaintenanceMaster.html?'+ 'deleteVehicleMaintenanceMaster', requestData, 'html');
...
function searchVehicleMaintenanceMaster(){
	...
	var data = {
		...
		var divName = '.content-page';
		var url = "VehicleMaintenanceMaster.html?searchVehicleMaintenanceMaster";
...
function ResetForm2(resetBtn){
	...
	resetForm(resetBtn);;
	var divName = '.content-page';
	var ajaxResponse = doAjaxLoading('VehicleMaintenanceMaster.html?AddVehicleMaintenanceMaster', {}, 'html',
			divName);
	$(divName).removeClass('ajaxloader');
	$(divName).html(ajaxResponse)	
	$('input[type=text]').val('');  
