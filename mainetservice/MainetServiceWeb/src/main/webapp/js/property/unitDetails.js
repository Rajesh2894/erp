...
$(document).ready(function(){
	...
	/*var a = $('#assdArea0').text();
	let totalPay= Number($('#assdArea0').text());
	$('#assdArea0').text(totalPay.toFixed(1));
	*/
		let totalTaxPayArrear= Number($('#totalTaxPayArrear').text());
	    $('#totalTaxPayArrear').text(totalTaxPayArrear.toFixed(1));
	    
	    let totalTaxPay= Number($('#totalTaxPay').text());
	    $('#totalTaxPay').text(totalTaxPay.toFixed(1));
	    
	    let billTotamount= Number($('#billTotamount').val());
	    $('#billTotamount').val(billTotamount.toFixed(1));
	  ...
	  $("#unitDetailTable tbody tr").each(
			    function(i) {
			    	...
			    	$('#assdArea' + i).text(totalPay.toFixed(1));
			     	...
			     	let alv= Number($('#alv' + i).text());
			    	$('#alv' + i).text(alv.toFixed(1));
			    	
			     	let maintananceCharge= Number($('#maintananceCharge' + i).text());
			    	$('#maintananceCharge' + i).text(maintananceCharge.toFixed(1));
			    	
			     	let rv= Number($('#rv' + i).text());
			    	$('#rv' + i).text(rv.toFixed(1));
...
function validateUnitDetailTable(errorList){
	...
	//var noOfDaysFlag =$("#noOfDaysEditableFlag").val();
	if(rowCount==3){
		reorderFirstRow();
	}
	$('.firstUnitRow').each(function(i) {	
...
//		var unitType=  $("#assdUnitTypeId"+i).val();
		...
		/* if(unitType=='0' || unitType == undefined){
		 ...
		 if(constructDateFlag != null && constructDateFlag == 'Y'){
...
*/			$(divName).html(returnData);
	...
	if ($("input:radio[name='offlineDTO.onlineOfflineCheck']").filter(":checked").val() == 'Y') {
	...
	else if (flag == 'BIF' && $("input:radio[name='offlineDTO.onlineOfflineCheck']").filter(":checked").val() == 'N'|| $("input:radio[name='offlineDTO.onlineOfflineCheck']").filter(":checked").val() == 'P')
		{
			return saveOrUpdateForm(element,"Bill Payment done successfully!", 'BifurcationForm.html?PrintReport', 'saveform');
		}
		...
		{
		 return saveOrUpdateForm(element,"Bill Payment done successfully!", 'AdminHome.html', 'saveform');
		}
...
function saveNoOfDaysEditProperty(element){
	...
	var object = __doAjaxRequest(
		"ChangeInAssessmentForm.html?saveNoOfDaysEditForm", 'POST',
		requestData, false, 'json');
	
	if (object.successFlag == "Y") {
		if(object.applicationId != null && object.applicationId != ''){
			showBoxForApproval(getLocalMessage("prop.application.edit.success.appNo" + object.applicationId));
		}else{
			showBoxForApproval(getLocalMessage("prop.application.edit.success"));
		}
	}else{
...
function showBoxForApproval(succesMessage) {
	...
	var totalpayableAmnt = $("#partialAmt").val();
	if (totalpayableAmnt > 0) {
		var redirectUrl = 'ChangeInAssessmentForm.html?PrintReport';
...
function reOrderChangeUnitTabIdSequence1(classNameFirst,classNamesecond){
...
//	$(this).find("select:eq(1)").attr("id", "assdUnitTypeId"+i);
	...
	if(ConstructFlag != null && ConstructFlag == 'Y'){
		...
		$(this).find("input:text:eq(2)").attr("id", "firstAssesmentDate"+i);
		$(this).find("input:text:eq(3)").attr("id", "taxableArea"+i);
		$(this).find("input:text:eq(4)").attr("id", "revisedPaidAmount"+i);
	}else{
		$(this).find("input:text:eq(1)").attr("id", "firstAssesmentDate"+i);
		$(this).find("input:text:eq(2)").attr("id", "taxableArea"+i);
		$(this).find("input:text:eq(3)").attr("id", "revisedPaidAmount"+i);
	}
...
//	$(this).find("select:eq(1)").attr("name","provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList["+i+"].assdUnitTypeId");
	...
	if(ConstructFlag != null && ConstructFlag == 'Y'){
		...
		$(this).find("input:text:eq(3)").attr("name","provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList["+i+"].assdBuildupArea");
		$(this).find("input:text:eq(4)").attr("name","provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList["+i+"].revisedPaidAmount");
		
		
	}else{
       $(this).find("input:text:eq(1)").attr("name","provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList["+i+"].firstAssesmentDate");
		$(this).find("input:text:eq(2)").attr("name","provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList["+i+"].assdBuildupArea");
		$(this).find("input:text:eq(3)").attr("name","provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList["+i+"].revisedPaidAmount");
	}
	$(this).find("select:eq(2)").attr("name","provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList["+i+"].assdConstruType");
...
//		$(this).find("select:eq(0)").attr("onchange", "occupancyTypeChange("+(j)+")");
		...
		$(this).find("select:eq(4)").attr("id", "natureOfProperty"+(utp+3));
		$(this).find("select:eq(5)").attr("id", "natureOfProperty"+(utp+4));
		//$(this).find("input:text:eq(1)").attr("id", "paidAmount"+j);	
...
//		$(this).find("input:text:eq(0)").attr("name","provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList["+j+"].assdAnnualRent");
		...
		$(this).find("select:eq(4)").attr("name","provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList["+j+"].assdNatureOfproperty4");
		$(this).find("select:eq(5)").attr("name","provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList["+j+"].assdNatureOfproperty5");
		//$(this).find("input:text:eq(1)").attr("name","provisionalAssesmentMstDto.provisionalAssesmentDetailDtoList["+j+"].paidAmount");
	});
