...
function saveOutstandingForm(obj) {
	...
	if (errorList.length > 0) {
		...
		var URL = 'DetailOutstandingRegister.html?GetDetailOutstanding';
...
$("#resetForm").on("click", function() {
	window.location.reload("#DetailOutstandingFormReport")
});
...
function saveXLSXOutstanding(obj){
	var errorList = [];
	var wardZone1 = $('#mnassward1').val();
	var wardZone2 = $("#mnassward2").val();
	var wardZone3 = $("#mnassward3").val();
	...
	if (errorList.length > 0) {
		...
		var URL = 'DetailOutstandingRegister.html?GetOutstandingXLSX';
		var returnData = __doAjaxRequest(URL, 'POST', requestData, false);
		if (returnData == "f") {
			errorList.push(getLocalMessage("property.report.finYear.date"));
			displayErrorsOnPage(errorList);
