...
function searchProperty() {
	...
	if (errorList.length == 0) {
		...
		var formAction	=	$('#SpecialNoticeGeneration').attr('action');
		var url=formAction+'?proceed';
		$('#SpecialNoticeGeneration').attr('action', url);
		$('#SpecialNoticeGeneration').submit();
...
function saveSpecialNotice(element){
	$('#datatables').DataTable().destroy();
	return saveOrUpdateForm(element,"Bill Payment done successfully!", 'SpecialNoticeGeneration.html', 'generateSpecialNotice');
}
...
function resetFormDetails(resetBtn) {
	var data = {};
	var URL = 'SpecialNoticeGeneration.html?resetFormData';
