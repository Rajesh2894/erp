...
function saveUsageForm(obj) {
	...
	if (mnFromdt == "") {
		errorList
				.push(getLocalMessage("property.validation.collection.from.date"));
	}
	if (mnTodt == "") {
		errorList
				.push(getLocalMessage("property.validation.collection.to.date"));
	}

	if (usageType1 == 0) {
		errorList.push(getLocalMessage("property.valid.usagetype"));
	}
	if ((compareDate(mnFromdt)) > (compareDate(mnTodt))) {
		errorList
				.push(getLocalMessage("property.validation.collection.from.to.date"));
	}
	...
	if (errorList.length > 0) {
		...
		var requestData = {
			...
			"mnTodt" : mnTodt,
			"proertyNo" : proertyNo,
			"usageType1" : usageType1
		}

		var URL = 'UsageWiseReport.html?GetUsageWiseReport';
...
function resetForm() {
	...
	$('input[type=text]').val('');
	$(".alert-danger").hide();
	$("#UsageWiseReportForm").validate().resetForm();
}
