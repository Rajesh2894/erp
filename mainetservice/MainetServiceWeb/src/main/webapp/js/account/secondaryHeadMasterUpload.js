var UPLOADURL = "tbAcSecondaryheadMaster.html"
...
function downloadTemplateForDataMap() {
	
	window.location.href = UPLOADURL + "?exportAccountSacHeadBankExcelData";
}
...
function uploadMapExcelFile(obj) {
	...
	if (errorList.length == 0) {
		$("#filePath").val(fileName);
		var url = UPLOADURL+"?loadMapExcelData";
		var formName = findClosestElementId(obj, 'form');
		var theForm = '#' + formName;
		var requestData = __serializeForm(theForm);
		var ajaxResponse = __doAjaxRequestForSave(url, 'post', requestData,
