...
$(function() {
	$("#grid").jqGrid(
			{
				url : "AccountTenderEntryAuthorization.html?getGridData",
				datatype : "json",
				mtype : "POST",
				colNames : [ '',getLocalMessage('accounts.workorder.no'),getLocalMessage('account.tenderentrydetails.tenderdate'),getLocalMessage('account.tenderentrydetails.tenderamount'),  getLocalMessage('accounts.emd.amount'), getLocalMessage('account.deposit.status'), getLocalMessage('bill.action')],
				colModel : [ 
...
$(function() {
		$(document).on('click', '.createData', function() {
			...
			var url = "AccountTenderEntryAuthorization.html?form";
...
function getDetails(i){
		...
		var url="AccountTenderEntryAuthorization.html?getDetails";
...
$(function() {
	$("#search").click(function(){ 
		...
		if( tenderId != '' ) {				
			var url = "AccountTenderEntryAuthorization.html?getTenderDetailsForEdit";
	...
	thisCount = count;
	 var primaryAcCode=$('#pacHeadId'+count).val();
		...
		if (primaryAcCode > 0) {
			...
			var json = __doAjaxRequest('AccountTenderEntryAuthorization.html?sacHeadItemsList','POST', postdata, false, 'json');
			var  optionsAsString='';
...
$(document).ready(function(){
	  ...
	  $('.tenderClass').each(function(i) {
	    	 ...
	    	 var json = __doAjaxRequest('AccountTenderEntryAuthorization.html?sacHeadItemsList',
...
function setBudgetaryProv(count)
{
	...
	var secondaryAcCode=$("sacHeadId="+count).val();
		...
		if(secondaryAccode>0){
				...
				var json=__doAjaxRequest('AccountTenderEntryAuthorization.html?getDetails','POST',postdata,false,'json');
				var  optionsAsString='';
...
function loadBudgetExpenditureData(obj) {
	...
	if (errorList.length == 0) {
		...
		var url = "AccountTenderEntryAuthorization.html?getBudgetExpenditureData";
...
function loadDepositTypeEmdData(obj) {
	...
	if (errorList.length == 0) {
		...
		var url = "AccountTenderEntryAuthorization.html?getDepositTypeEmdData";
...
function getOrgBalAmount(cnt) {
	...
	if (errorList.length == 0) {
		...
		var url = "AccountTenderEntryAuthorization.html?getOrgBalGridload&cnt="+cnt;
...
function redirectToTenderEntryHomePage () {
	//$.fancybox.close();
	window.location.href='AccountTenderEntryAuthorization.html';
}
...
function searchWorkOrderEntryData(){
	...
	var url ="AccountTenderEntryAuthorization.html?getjqGridsearch";
	var requestData = {
