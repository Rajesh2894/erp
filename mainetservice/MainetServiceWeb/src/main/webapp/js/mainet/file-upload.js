									...
									if(file.length != 0){  
										...
										if( ($(this).attr('checklistDocSize')!="" && fileSize <= parseInt($(this).attr('checklistDocSize'))) || ($(this).attr('maxsize')!= "" && fileSize <= parseInt($(this).attr('maxsize')))){
											...
											for (var i = 0; i < file.length; i++) {
												...
												if(flag == 0){
													$('.error-dialog').html('<p class="text-center padding-10 text-bold margin-top-20">' +getLocalMessage("Common.file.validation.error.msg.only") + $(this).attr('extension') +"&nbsp"+ getLocalMessage("Common.file.validation.error.msg.allow")+ '</p>');
											...
											let docSize=$(this).attr('maxsize') != "" ? parseInt($(this).attr('maxsize')) :parseInt($(this).attr('checklistDocSize')); 
											   ...
											   if(sizeInMB >= 1){
												   $('.error-dialog').html('<p class="text-center padding-10 text-bold margin-top-20">' + getLocalMessage("Common.file.validation.error.msg.upload") + sizeInMB + " MB." + '</p>');
											   }
											   else{
												   $('.error-dialog').html('<p class="text-center padding-10 text-bold margin-top-20">' + getLocalMessage("Common.file.validation.error.msg.upload") + sizeInMB*1000 + " KB." + '</p>');
											   }
											   showModalBox('.error-dialog');
								     		   $(this).val(null);
								     		   return false;
