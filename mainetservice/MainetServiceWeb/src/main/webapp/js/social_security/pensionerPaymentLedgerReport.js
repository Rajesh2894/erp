...
$(document).ready(function() {
	...
	$("#resetForm").on("click", function() {
		window.location.reload("#PensionerFormReport")
	});
});
...
function saveForm(obj) {
	...
	if (errorList.length > 0) {
		...
		var requestData = '&pSchemeName=' + pSchemeName + '&pFrmDate='
				+ pFrmDate + '&pToDate=' + pToDate;

		var URL = 'PensionerPayment.html?GetPensionerPayment';
		var returnData = __doAjaxRequest(URL, 'POST', requestData, false);
		window.open(returnData, '_blank');
		
		/*if (returnData == "f") {
			errorList.push(getLocalMessage("social.report.validation.finYrDate"));
			displayErrorsOnPage(errorList);
		} else {
			window.open(returnData, '_blank');
		}*/
