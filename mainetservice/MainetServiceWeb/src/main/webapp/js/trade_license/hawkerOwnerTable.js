	...
	if (errorList == 0)
		$("#ownerDetail tbody tr").each(function(i) {
			...
			if(rowCount<=2){
							...
							var troEducation = $("#troEducation" + i).val();
							var troCast = $("#troCast" + i).val();
							var troAge = $("#troAge" + i).val();
				...
				var constant = i+1;
							...
							if (troAge == "" || troAge == undefined
									|| troAge == "0") {
								errorList
										.push(getLocalMessage("Hawkerlicense.validation.troAge")
												+" " +constant);
							}
							...
							//105332
							/*if (troEducation == "" || troEducation == undefined
									|| troEducation == "0") {
								errorList
										.push(getLocalMessage("Hawkerlicense.validation.troEducation")
												+" " +constant);
							}*/
							
							
							
							if (troCast == "" || troCast == undefined
									|| troCast == "0") {
								errorList
										.push(getLocalMessage("Hawkerlicense.validation.troCast")
												+" " +constant);
							}
							...
							else if (ownerAdharNo.length !=0 && ownerAdharNo.length<12){
...
function reOrderOwnerDetailsSequence(classNameFirst) {
	...
	$(classNameFirst).each(
		function(i) 
		{
			...
			$(this).find("input:text:eq(2)").attr("id", "troAge" + i);
			$(this).find("textarea:eq(0)").attr("id", "troAddress" + i);
			$(this).find("input:text:eq(3)").attr("id", "troMobileno" + i);
			$(this).find("input:text:eq(4)").attr("id", "troEmailid" + i);
			$(this).find("input:text:eq(5)").attr("id", "troAdhno" + i);
			$(this).find("input:text:eq(6)").attr("id", "troEducation" + i);
			$(this).find("input:text:eq(7)").attr("id", "troCast" + i);
			...
			$(this).find("input:text:eq(2)").attr("name", "tradeMasterDetailDTO.tradeLicenseOwnerdetailDTO["+i+"].troAge");
			$(this).find("textarea:eq(0)").attr("name", "tradeMasterDetailDTO.tradeLicenseOwnerdetailDTO["+i+"].troAddress");
			$(this).find("input:text:eq(3)").attr("name", "tradeMasterDetailDTO.tradeLicenseOwnerdetailDTO["+i+"].troMobileno");
			$(this).find("input:text:eq(4)").attr("name", "tradeMasterDetailDTO.tradeLicenseOwnerdetailDTO["+i+"].troEmailid");
			$(this).find("input:text:eq(5)").attr("name", "tradeMasterDetailDTO.tradeLicenseOwnerdetailDTO["+i+"].troAdhno");
			$(this).find("input:text:eq(6)").attr("name", "tradeMasterDetailDTO.tradeLicenseOwnerdetailDTO["+i+"].troEducation");
			$(this).find("input:text:eq(7)").attr("name", "tradeMasterDetailDTO.tradeLicenseOwnerdetailDTO["+i+"].troCast");
...
function fileCountUpload() {
	...
	var response = __doAjaxRequest('HawkerLicenseApplicationForm.html?fileCountUpload',
