									...
									errorList
								    	...
								    	if (emailId !="")		
										{
										  ...
										  var valid = emailRegex.test(emailId);
										   if (!valid) {
											   errorList.push(getLocalMessage('agency.registration.validate.agency.emailId.valid')+" " +constant);
										   } 
										}
								    }
...
function reOrderOwnerDetailsSequence(appendableClass) {
	...
	$(appendableClass).each(
		function(i) 
		{
			...
			/*$(this).find("input:text:eq(0)").attr("name", "agencyRequestDto.masterDto.agencyOwner");
			$(this).find("input:text:eq(1)").attr("name", "agencyRequestDto.masterDto.agencyContactNo");
			$(this).find("input:text:eq(2)").attr("name", "agencyRequestDto.masterDto.agencyEmail");
			$(this).find("input:text:eq(3)").attr("name", "agencyRequestDto.masterDto.uidNo");
			$(this).find("input:text:eq(4)").attr("name", "agencyRequestDto.masterDto.panNumber");*/
