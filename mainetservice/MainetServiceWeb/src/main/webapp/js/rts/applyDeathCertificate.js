$(document).ready(function() {
	prepareDateTag();
	$("#frmDeathCertificateForm").validate({
...
function saveDeathCertificateData(element) {
	...
	if (errorList.length > 0) {
		...
		displayErrorsOnPage(errorList);
		 ...
		 saveOrUpdateForm(element, "", 'applyForDeathCertificate.html?printAgencyRegAckw', 'saveform');
		...
		return saveOrUpdateForm(element, "", 'applyForDeathCertificate.html?PrintReport', 'saveform');
	  }
	 }
...
function resetMemberMaster(element){
	...
	var ajaxResponse = doAjaxLoading('applyForDeathCertificate.html?resetDeathForm', {}, 'html',
			divName);
