...
import com.abm.mainet.securitymanagement.ui.model.EmployeeAttendanceModel;

@Controller
@RequestMapping(value = "/EmployeeAttendanceEntry.html")
public class EmployeeAttendanceController extends AbstractFormController<EmployeeAttendanceModel>{
	...
	@ResponseBody
	@RequestMapping(params = "getEmpForAttendance", method = RequestMethod.POST)
	public ModelAndView  getEmpForAttendance(final HttpServletRequest httpServletRequest,Model model,
		...
		this.getModel().getDetList().clear();
		String empCode=null;
		...
		if (!employeeScheList.isEmpty()) {
			if (!employeeScheList.get(0).getEmplDetDto().isEmpty()) {
				...
				if (employeeScheList.get(0).getEmplDetDto() != null) {
					/*employeeScheList.get(0).getEmplDetDto().forEach(d -> {
						if (d.getAttStatus() == null || d.getAttStatus().isEmpty()) {
							empdetList.add(d);
						}
					});*/
		...
		ModelAndView mv = new ModelAndView("EmployeeAttendanceValidn", MainetConstants.FORM_NAME, getModel());
