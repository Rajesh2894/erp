...
public class EmployeeAttendanceModel extends AbstractFormModel{
	...
	public boolean saveForm() {
		...
		for (int i = 0; i < detList.size(); i++) {
			...
			detList.get(i).setLgIpMacUpd(lgIpMac);
			detList.get(i).setCreatedBy(createdBy);
			detList.get(i).setEmplScdlId(employeeSchedulingDTOList.get(0).getEmplScdlId());
			
		}
		employeeSchedulingService.saveData(detList);
		this.setSuccessMessage(ApplicationSession.getInstance().getMessage("Data Saved Successfully"));
		return true;
