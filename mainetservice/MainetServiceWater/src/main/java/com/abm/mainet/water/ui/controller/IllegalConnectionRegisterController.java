...
import com.abm.mainet.water.ui.model.IllegalConnectionRegisterdModel;

@Controller
@RequestMapping("/llegalConnection.html")
public class IllegalConnectionRegisterController extends AbstractFormController<IllegalConnectionRegisterdModel> {
	@Autowired
	...
	@RequestMapping(params = "GetIllegalReport", method = { RequestMethod.POST, RequestMethod.GET })
	public @ResponseBody String viewReportSheet(
			...
			@RequestParam("csTodt") Date csTodt, @RequestParam("ConnType") Long ConnType,
			@RequestParam("MeteredType") Long MeteredType, @RequestParam("CcnSize") Long CcnSize,
		...
		if (error == null) {
			return ServiceEndpoints.WATER_BIRT_REPORT_URL + "=IllegalConnectionRegister.rptdesign&OrgId=" + currentOrgId
					...
					+ "&FromDate=" + fromdt + "&ToDate=" + todt + "&TariffCategory=" + tarrifType + "&FromDate="
					+ fromdt + "&ToDate=" + todt + "&ConnectionType=" + ConnType + "&MeterType=" + MeteredType
					+ "&ConnectionSize=" + CcnSize;
