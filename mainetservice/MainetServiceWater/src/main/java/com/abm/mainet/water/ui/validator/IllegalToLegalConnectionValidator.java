...
public class IllegalToLegalConnectionValidator extends BaseEntityValidator<IllegalToLegalConnectionModel> {
	...
	protected void performValidations(final IllegalToLegalConnectionModel model,
			EntityValidationContext<IllegalToLegalConnectionModel> entityValidationContext) {
		...
		if ("Y".equalsIgnoreCase(panCheck)) {
			if (model.getReqDTO().getApplicantDTO().getPanNo() == null
					|| model.getReqDTO().getApplicantDTO().getPanNo().isEmpty()) {
				...
				String[] validatePan = { "C", "H", "F", "A", "T", "B", "L", "J", "G" };
				boolean check = false;
