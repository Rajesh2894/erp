...
@Controller
@RequestMapping("/PlumberRenewalAuth.html")
public class PlumberRewalAuthController extends AbstractFormController<PlumberLicenseFormModel> {
	private static final Logger LOGGER = LoggerFactory.getLogger(PlumberRewalAuthController.class);
	...
	public ModelAndView executeLicenseRenewal(@RequestParam("appNo") final Long applicationId,
		...
		return new ModelAndView("PlumberRewalAuth", MainetConstants.CommonConstants.COMMAND, getModel());
	...
	public ModelAndView viewDetails(@RequestParam("appNo") final String applicationId,
        ...
        return new ModelAndView("PlumberRenewalAuthView", MainetConstants.FORM_NAME, getModel());
	}
	...
	@Override
	@RequestMapping(params = "saveAuthRenewData", method = RequestMethod.POST)
	public ModelAndView saveform(final HttpServletRequest httpServletRequest) {
		...
		if (decision.equalsIgnoreCase(MainetConstants.WorkFlow.Decision.APPROVED) && lastProcess
			...
			if (model.savePlumberRenewalLoiData(applicationId, service.getSmServiceId(), orgId,
					UserSession.getCurrent().getEmployee().getEmpId(), model.getServiceCode(),
					model.getPlumberLicenseReqDTO().getPlumLicToDate())) {
			 ...
			 * MainetConstants.URLBasedOnShortCode.PLCA.getUrl(); String type =
			 * PrefixConstants.SMS_EMAIL_ALERT_TYPE.REJECTED; String bytes =
			 * plumberLicenseService.generateJasperReportPDF(model.getPlumberLicenseReqDTO()
			 * , outputStream, oParms, fileName, menuURL, type);
			 */
			/*
			 * plumberLicenseService.sendSMSandEMail(applicantDto, applicationNo, payAmount,
			 * serviceShortCode, organisation);
			 */ respObj = JsonViewObject
	...
	public ModelAndView printRejAppReport(final HttpServletRequest httpServletRequest,
	   ...
	   @RequestMapping(params = "checkLastApproval", method = RequestMethod.POST)
	    public ModelAndView checkLastApproval(HttpServletRequest httpServletRequest) {
	        getModel().bind(httpServletRequest);
	        PlumberLicenseFormModel model = this.getModel();
	        ModelAndView mv = new ModelAndView("PlumberRewalAuth", MainetConstants.CommonConstants.COMMAND, getModel());
	        ...
	        boolean lastApproval = iWorkFlowTypeService.isLastTaskInCheckerTaskList(model.getWorkflowActionDto().getTaskId());
	        if (lastApproval
