...
@Controller("bndBirthCertificateController")
@RequestMapping("bndApplyforBirthCertificate.html")
public class BirthCertificateController extends AbstractFormController<BirthCertificateModel> {
	...
	public ModelAndView index(final Model model, final HttpServletRequest httpServletRequest,
		...
		this.getModel().setCommonHelpDocs("bndApplyforBirthCertificate.html");
		...
		if(requestDTO.getApplicationId()!=null)
        {
        	...
        	this.getModel().getBirthCertificateDto().setDateOfBirth(Utility.dateToString(this.getModel().getBirthCertificateDto().getBrDob()));
	...
	public ModelAndView confirmToProceed(HttpServletRequest httpServletRequest,
		...
		model.setCommonHelpDocs("bndApplyforBirthCertificate.html");
	...
	public ModelAndView printBndRegAcknowledgement(HttpServletRequest request) {
		...
		LinkedHashMap<String, Object> map = iBirthRegSevice
				.serviceInformation(UserSession.getCurrent().getOrganisation().getOrgid(), RtsConstants.RBC);
		BndAcknowledgementDto ackDto = new BndAcknowledgementDto();
		ackDto.setApplicationId(Long.valueOf(birthModel.getBirthCertificateDto().getApmApplicationId()));
	...
	private void fetchCheckListForAck(BirthCertificateModel birthRegModel, LinkedHashMap<String, Object> map) {
		List<DocumentDetailsVO> docs = null;
		final Long orgId = UserSession.getCurrent().getOrganisation().getOrgid();
		final WSRequestDTO requestDTO = new WSRequestDTO();
		requestDTO.setModelName(RtsConstants.CHECKLIST_MODEL);
	...
	@RequestMapping(params = "birthCertificateView", method = { RequestMethod.POST })
	public ModelAndView birthCertificateView(final Model model, HttpServletRequest request) {
		sessionCleanup(request);
		Long orgId = UserSession.getCurrent().getOrganisation().getOrgid();

		ModelAndView mv = null;
		String viewName = "ViewBirthCertificateDetails";
	...
	@ResponseBody
	@RequestMapping(params = "searchBirthDetails", method = RequestMethod.POST)
	public List<BirthCertificateDTO> searchBirthDetails(HttpServletRequest request) {
		BirthCertificateModel model = this.getModel();
		model.bind(request);
		model.getBirthCertificateDto().setOrgId(Utility.getOrgId());
		List<BirthCertificateDTO> searchConnectionDetails = birthCertificateService
				.searchBirthCertiDetails(model.getBirthCertificateDto());
		if(null!=searchConnectionDetails)
			this.getModel().setSearchBirthCertiDtoList(searchConnectionDetails);
		
		this.getModel().getSearchBirthCertiDtoList().forEach(tr->{
			tr.getUploadDocument().forEach(doc->{
				if(tr .getBrDownloadPath()==null)
				 tr.setBrDownloadPath(Utility.getImageDetails((doc.getUploadedDocumentPath()+"\\"+doc.getDoc_DESC_ENGL()).replace("\\", "\\\\")));				 
			});
		});
	...
	@RequestMapping(params = "viewBirthCertificate", method = { RequestMethod.POST })
	public ModelAndView viewBirthCertificate(@RequestParam("apmApplicationId") final Long apmApplicationId,
			final HttpServletRequest request) {
		bindModel(request);
		
		if(this.getModel().getSearchBirthCertiDtoList()!=null) {
			List<BirthCertificateDTO> birthDtoList = this.getModel().getSearchBirthCertiDtoList().stream().filter(dtoList -> (dtoList.getApmApplicationId()!=null 
					&& dtoList.getApmApplicationId().equals(apmApplicationId))).collect(Collectors.toList());
				
			if(CollectionUtils.isNotEmpty(birthDtoList) && birthDtoList.get(0)!=null)
				this.getModel().setBirthCertificateDto(birthDtoList.get(0));
		}
		
		return new ModelAndView("ViewBirthCertificateDetForm", "command", getModel());
	}
