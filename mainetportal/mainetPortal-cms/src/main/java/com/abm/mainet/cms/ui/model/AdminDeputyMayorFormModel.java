...
@Component
@Scope("session")
public class AdminDeputyMayorFormModel extends AbstractEntryFormModel<ProfileMaster> implements Serializable {

    private static final long serialVersionUID = -3772636431764660190L;
    private static final Logger LOG = Logger.getLogger(AdminDeputyMayorFormModel.class);
    @Autowired
    ...
    private String eipHomeImagesPath = UserSession.getCurrent().getOrganisation().getOrgid() + File.separator + date
            + File.separator
            + MainetConstants.DirectoryTree.EIP + File.separator + "HOME_IMAGES" + File.separator + "DEPUTYMAYOR" + File.separator
    ...
    public void viewForm(final long rowId) {
        setMode("view");
        final ProfileMaster profileMaster = iProfileMasterService.getProfileMasterByViewId(rowId);
        setProfileMaster(profileMaster);
        setEntity(profileMaster);
    }
    ...
    public boolean saveOrUpdateForm() {
        validateBean(getProfileMaster(), AdminProfileMasterValidator.class);
        ...
        Utility.sendSmsAndEmail(getAppSession().getMessage("dashboard.committee2")+" "+(UserSession.getCurrent().getLanguageId()==1?entity.getpNameEn():entity.getpNameReg()),entity.getChekkerflag(),id,entity.getUpdatedBy());
        return true;
    }
