...
@Component
@Scope("session")
public class AdminCorporatorsFormModel extends AbstractEntryFormModel<ProfileMaster> implements Serializable {
    ...
    private boolean saveCorporatorsForm() throws FrameworkException {
    ...
    private boolean updateCorporatorsForm() throws FrameworkException {
    ...
    public boolean saveOrUpdateForm() {
        ...
        if (getMode().equals(MainetConstants.Transaction.Mode.ADD)) {
            if (saveCorporatorsForm()) {
                return true;
            } else {
                return false;
            }
        } else {
            if (updateCorporatorsForm()) {
                return true;
            } else {
                return false;
            }
    ...
    private LookUp getProfileMasterSection(final String prefix) {
        ...
        for (final LookUp lookUp2 : lookUps) {
            ...
            if (lookUp.getLookUpCode().equals(PrefixConstants.Prefix.CITZEN)) {
                quickLink = lookUp;
                break;
            }
