...
import com.abm.mainet.cms.ui.validator.HelpDocValidator;
...
public class HelpDocModel extends AbstractEntryFormModel<CommonHelpDocs> implements Serializable {
    ...
    private static final Logger LOG = Logger.getLogger(HelpDocModel.class);
    ...
    public String getDirectoryTree() {
        ...
        return UserSession.getCurrent().getOrganisation().getOrgid() + File.separator + date + MainetConstants.FILE_PATH_SEPARATOR
                + MainetConstants.HelpDoc.HELPDOC + MainetConstants.FILE_PATH_SEPARATOR + getEntity().getModuleName();
    }
    ...
    public boolean saveForm() {
        ...
        if (getModeOfType().equalsIgnoreCase("A") && !getEntity().getModuleName().equalsIgnoreCase(MainetConstants.BLANK)) {
            ...
            final boolean resultmoduleName = iHelpDocService.checkExistingModuleName(getEntity().getModuleName(), UserSession.getCurrent().getOrganisation());
