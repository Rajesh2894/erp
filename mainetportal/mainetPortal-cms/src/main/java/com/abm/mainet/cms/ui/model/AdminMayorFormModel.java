...
@Component
@Scope("session")
public class AdminMayorFormModel extends AbstractEntryFormModel<ProfileMaster> implements Serializable {

    private static final long serialVersionUID = 2121016192299328696L;
    private static final Logger LOG = Logger.getLogger(AdminMayorFormModel.class);
    ...
    private String eipHomeImagesPath = UserSession.getCurrent().getOrganisation().getOrgid() + File.separator + date
            + File.separator
            + MainetConstants.DirectoryTree.EIP + File.separator + "HOME_IMAGES" + File.separator + "MAYOR" + File.separator
    ...
    public boolean saveOrUpdateForm() {
        ...
        iProfileMasterService.saveOrUpdate(entity, entity.getChekkerflag1());
        Utility.sendSmsAndEmail(getAppSession().getMessage("dashboard.committee1")+" "+(UserSession.getCurrent().getLanguageId()==1?entity.getpNameEn():entity.getpNameReg()),entity.getChekkerflag(),id,entity.getUpdatedBy());
        return true;
