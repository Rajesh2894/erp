...
public class DuplicatePlumberLicenseModel extends AbstractFormModel implements Serializable {
	...
	public boolean saveForm() {
		try {
			...
			outPutObject = iPlumberLicenseFormService.saveDuplicatePlumberData(requestDTO);
			...
			if ((outPutObject != null) && (outPutObject.getApplicationId() != null)) {
				...
				throw new FrameworkException(
						"Sorry,Your application for Duplicate License has not been saved due to some technical problem.");
			}
		} catch (final Exception e1) {
			throw new FrameworkException(
					"Sorry,Your application for Duplicate License has not been saved due to some technical problem.", e1);
		}
		return true;
	}
