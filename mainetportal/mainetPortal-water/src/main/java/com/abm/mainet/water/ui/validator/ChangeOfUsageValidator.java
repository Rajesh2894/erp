...
@Component
public class ChangeOfUsageValidator extends
        BaseEntityValidator<ChangeOfUsageRequestDTO> {

    @Override
    protected void performValidations(
            final ChangeOfUsageRequestDTO model,
            final EntityValidationContext<ChangeOfUsageRequestDTO> entityValidationContext) {
        entityValidationContext.rejectIfEmpty(model
                .getChangeOfUsage().getRemark(),
                "changeOfUsage.remark");
