...
public class MoibleRequestValidatorFilter extends GenericFilterBean {
  ...
  private static final Logger LOG = Logger.getLogger(MoibleRequestValidatorFilter.class);
    ...
    try {
      ...
      if (httpRequest.getHeader("X_REQUESTED_BY") != null && httpRequest.getHeader("X_REQUESTED_BY").indexOf("Mobile") != -1) {
        if (httpRequest.getHeader(JwtUtil.getJwtToken()) != null) {
          EmployeeBean user =
              jwtUtil.parseToken((String) httpRequest.getHeader(JwtUtil.getJwtToken()));
  ...
  public Boolean isEmployeeSessionValid(EmployeeBean user, HttpServletRequest httpRequest) {
    LOG.info(JwtUtil.getJwtToken() + user.getEmpId() +  ">>isEmployeeSessionValid>>" + httpRequest.getHeader("uid"));
    LOG.info(JwtUtil.getJwtToken() + user.getOrgid()+  ">>isEmployeeSessionValid>>" + httpRequest.getHeader("oid"));
    Long orgId = Long.valueOf(httpRequest.getHeader("oid"));
    Long userId = Long.valueOf(httpRequest.getHeader("uid"));
    boolean validSession = false;
    if (orgId == user.getOrgid() && userId == user.getEmpId()) {
      validSession = true;
    }
    return validSession;
