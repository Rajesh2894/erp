...
public class SMSAndEmailTempalteValidator extends BaseEntityValidator<SMSAndEmailInterface> {
    ...
    protected void performValidations(final SMSAndEmailInterface entity,
        ...
        if ((entity.getServiceId() != null) && (entity.getServiceId().getServiceId() != null)
                && (entity.getServiceId().getServiceId() == 0)) {
            ...
            if (entity.getAlertType().equalsIgnoreCase(MainetConstants.MENU.S)) {
                ...
                if (MainetConstants.NON.equals(check)) {
                    ...
                    if ((entity.getSmsAndmailTemplate().getSmsBody() == null)
                            || entity.getSmsAndmailTemplate().getSmsBody().isEmpty()) {
                        validationContext.addOptionConstraint(getApplicationSession().getMessage("eipsmsemail.smsbody1"));
                    } else {
                        if (!Pattern.compile(MainetConstants.RegEx.ENGLISH_REG_EX)
                            ...
                            validationContext.addOptionConstraint(getApplicationSession()
                                    .getMessage("eipsmsemail.smsbody1en"));
                        } else if ((entity.getSmsAndmailTemplate().getSmsBody().replaceAll("\\W+", MainetConstants.BLANK).trim()).isEmpty()) {
                            validationContext.addOptionConstraint(getApplicationSession().getMessage("eipsmsemail.smsbody1"));
                        }
                    }
                    if ((entity.getSmsAndmailTemplate().getSmsBodyReg() == null)
                            || entity.getSmsAndmailTemplate().getSmsBodyReg().isEmpty()) {
                        validationContext.addOptionConstraint(getApplicationSession().getMessage("eipsmsemail.smsbody2"));
                    } else {
                        if (!Pattern.compile(MainetConstants.RegEx.ENGLISH_REG_EX)
                                .matcher(entity.getSmsAndmailTemplate().getSmsBodyReg()).matches()) {
                            validationContext.addOptionConstraint(getApplicationSession()
                                    .getMessage("eipsmsemail.smsbody2mar"));
                        } else if ((entity.getSmsAndmailTemplate().getSmsBodyReg().replaceAll("\\W+", MainetConstants.BLANK).trim()).isEmpty()) {
                            validationContext.addOptionConstraint(getApplicationSession().getMessage("eipsmsemail.smsbody2"));
                        }
                    }
                ...
                if ("No".equals(check)) {
                    if ((entity.getSmsAndmailTemplate().getMailBody() == null)
                            || entity.getSmsAndmailTemplate().getMailBody().isEmpty()) {
                        validationContext.addOptionConstraint(getApplicationSession().getMessage("eipsmsemail.mailbody1"));
                    } else {
                        if (!Pattern.compile(MainetConstants.RegEx.ENGLISH_REG_EX)
                                .matcher(entity.getSmsAndmailTemplate().getMailBody()).matches()) {
                            validationContext.addOptionConstraint(getApplicationSession()
                                    .getMessage("eipsmsemail.mailbody1en"));
                        } else if ((entity.getSmsAndmailTemplate().getMailBody().replaceAll("\\W+", MainetConstants.BLANK).trim()).isEmpty()) {
                            validationContext.addOptionConstraint(getApplicationSession().getMessage("eipsmsemail.mailbody1"));
                        }
                    }
                    if ((entity.getSmsAndmailTemplate().getMailSub() == null)
                            || entity.getSmsAndmailTemplate().getMailSub().isEmpty()) {
                        validationContext.addOptionConstraint(getApplicationSession().getMessage("eipsmsemail.mailsub1"));
                    } else {
                        if (!Pattern.compile(MainetConstants.RegEx.ENGLISH_REG_EX)
                                .matcher(entity.getSmsAndmailTemplate().getMailSub()).matches()) {
                            validationContext.addOptionConstraint(getApplicationSession()
                                    .getMessage("eipsmsemail.mailsub1en"));
                        } else if ((entity.getSmsAndmailTemplate().getMailSub().replaceAll("\\W+", MainetConstants.BLANK).trim()).isEmpty()) {
                            validationContext.addOptionConstraint(getApplicationSession().getMessage("eipsmsemail.mailsub1"));
                        }
                    }

                    if ((entity.getSmsAndmailTemplate().getMailBodyReg() == null)
                            || entity.getSmsAndmailTemplate().getMailBodyReg().isEmpty()) {
                        validationContext.addOptionConstraint(getApplicationSession().getMessage("eipsmsemail.mailbody2"));
                    } else {
                        if (!Pattern.compile(MainetConstants.RegEx.ENGLISH_REG_EX)
                                .matcher(entity.getSmsAndmailTemplate().getMailBodyReg()).matches()) {
                            validationContext.addOptionConstraint(getApplicationSession()
                                    .getMessage("eipsmsemail.mailbody2mar"));
                        } else if ((entity.getSmsAndmailTemplate().getMailBodyReg().replaceAll("\\W+", MainetConstants.BLANK).trim()).isEmpty()) {
                            validationContext.addOptionConstraint(getApplicationSession().getMessage("eipsmsemail.mailbody2"));
                        }
                    }
                    if ((entity.getSmsAndmailTemplate().getMailSubReg() == null)
                            || entity.getSmsAndmailTemplate().getMailSubReg().isEmpty()) {
                        validationContext.addOptionConstraint(getApplicationSession().getMessage("eipsmsemail.mailsub2"));
                    } else {
                        if (!Pattern.compile(MainetConstants.RegEx.ENGLISH_REG_EX)
                                .matcher(entity.getSmsAndmailTemplate().getMailSubReg()).matches()) {
                            validationContext.addOptionConstraint(getApplicationSession()
                                    .getMessage("eipsmsemail.mailsub2mar"));
                        } else if ((entity.getSmsAndmailTemplate().getMailSubReg().replaceAll("\\W+", MainetConstants.BLANK).trim()).isEmpty()) {
                            validationContext.addOptionConstraint(getApplicationSession().getMessage("eipsmsemail.mailsub2"));
                        }
                    }
                } else {
                    if ((entity.getSmsAndmailTemplate().getMailBody() == null)
                            || entity.getSmsAndmailTemplate().getMailBody().equals(MainetConstants.BLANK)) {
                        validationContext.addOptionConstraint("eipsmsemail.mailbody1");
                    }
                    if ((entity.getSmsAndmailTemplate().getMailSub() == null)
                            || entity.getSmsAndmailTemplate().getMailSub().equals(MainetConstants.BLANK)) {
                        validationContext.addOptionConstraint("eipsmsemail.mailsub1");
                    }
                    if ((entity.getSmsAndmailTemplate().getMailBodyReg() == null)
                            || entity.getSmsAndmailTemplate().getMailBodyReg().equals(MainetConstants.BLANK)) {
                        validationContext.addOptionConstraint("eipsmsemail.mailbody2");
                    }
                    if ((entity.getSmsAndmailTemplate().getMailSubReg() == null)
                            || entity.getSmsAndmailTemplate().getMailSubReg().equals(MainetConstants.BLANK)) {
