...
/**
 * Implementation of {@link org.springframework.http.converter.HttpMessageConverter HttpMessageConverter} that can read and write
 * JSON using <a href="http://jackson.codehaus.org/">Jackson's</a> {@link ObjectMapper}.
 *
 * <p>
...
public class MappingJackson2HttpMessageConverter extends AbstractHttpMessageConverter<Object> {
    ...
    public static final Charset DEFAULT_CHARSET = Charset.forName(MainetConstants.UTF8);
     ...
     * <p>
     * Setting a custom-configured {@code ObjectMapper} is one way to take further control of the JSON serialization process. For
     * example, an extended {@link org.codehaus.jackson.map.SerializerFactory} can be configured that provides custom serializers
     * for specific types. The other option for refining the serialization process is to use Jackson's provided annotations on the
     ...
     * syntactically invalid as a script so that it cannot be hijacked. This prefix does not affect the evaluation of JSON, but if
     * JSON validation is performed on the string, the prefix would need to be ignored.
     */
    ...
    protected void writeInternal(final Object object, final HttpOutputMessage outputMessage)
        ...
        final JsonGenerator jsonGenerator = objectMapper.getJsonFactory().createJsonGenerator(outputMessage.getBody(), encoding);
        try {
            if (prefixJson) {
     ...
     * <p>
     * The default implementation returns {@link TypeFactory#type(java.lang.reflect.Type)}, but this can be overridden in
