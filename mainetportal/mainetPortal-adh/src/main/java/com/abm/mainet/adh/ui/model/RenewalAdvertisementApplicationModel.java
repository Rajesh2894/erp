...
public class RenewalAdvertisementApplicationModel extends AbstractFormModel {
    ...
    public boolean saveApplication() {
        ...
        advertisementReqDto.setDocumentList(getFileUploadList(getCheckList(), FileUploadUtility.getCurrent()
                .getFileMap()));
        advertisementReqDto.setNewOrRenewalApp(MainetConstants.AdvertisingAndHoarding.RENEWAL);
        if (advertisementReqDto.getAdvertisementDto().getCreatedBy() != null) {
            ...
            advertisementReqDto.getAdvertisementDto().setLgIpMacUpd(lgIp);
            advertisementReqDto.getAdvertisementDto().setUpdatedBy(createdBy);
            advertisementReqDto.getAdvertisementDto().setUpdatedDate(newDate);
            advertisementReqDto.setApplicantDetailDTO(advertisementReqDto.getApplicantDetailDTO());
            final PortalService serviceMasterData = serviceMaster.getPortalServiceMaster(
    				MainetConstants.AdvertisingAndHoarding.RENEWAL_SERVICE_CODE, orgId);
            ...
            if (!advertisementReqDto.getAdvertisementDto().getNewAdvertDetDtos().isEmpty()) {
                advertisementReqDto.getAdvertisementDto().getNewAdvertDetDtos().forEach(newAdvApp -> {
                    newAdvApp.setUpdatedBy(createdBy);
                    newAdvApp.setUpdatedDate(newDate);
                    newAdvApp.setOrgId(orgId);
                    newAdvApp.setLgIpMacUpd(lgIp);
                });
            }
        }
    ...
    private void setAndSaveChallanDto(CommonChallanDTO offline, NewAdvertisementReqDto newAdvertisementReqDto) {
        ...
        if ((offline.getOnlineOfflineCheck() != null) && offline.getOnlineOfflineCheck().equals(MainetConstants.FlagY)) {
            setPaymentMode(MainetConstants.AdvertisingAndHoarding.ONLINE);
        }
        if ((offline.getOnlineOfflineCheck() != null) &&
                offline.getOnlineOfflineCheck().equals(MainetConstants.FlagN)) {
	...
	public void redirectToPayDetails(final HttpServletRequest httpServletRequest,
		...
		payURequestDTO.setUdf5(MainetConstants.AdvertisingAndHoarding.RENEWAL_SERVICE_CODE);
