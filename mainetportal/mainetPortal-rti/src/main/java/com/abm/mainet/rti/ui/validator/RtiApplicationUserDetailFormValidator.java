...
@Component
public class RtiApplicationUserDetailFormValidator extends BaseEntityValidator<RtiApplicationUserDetailFormModel> {
	/*
	 * @Resource private IWorkflowTyepResolverService iWorkflowTyepResolverService;
	 */

	@Autowired
	private IRtiApplicationDetailService iRtiDetailsService;

	@Override
	protected void performValidations(RtiApplicationUserDetailFormModel model,
			EntityValidationContext<RtiApplicationUserDetailFormModel> entityValidationContext) {
		...
		if (!Utility.isEnvPrefixAvailable(UserSession.getCurrent().getOrganisation(), MainetConstants.APP_NAME.DSCL)&&(model.getReqDTO().getGender() == null) || (model.getReqDTO().getGender().isEmpty())) {
		...
		if (!Utility.isEnvPrefixAvailable(UserSession.getCurrent().getOrganisation(), MainetConstants.APP_NAME.DSCL)&&(model.getReqDTO().getRtiLocationId() == 0)) {
		...
		if ((model.getReqDTO().getIsBPL() == null) || (model.getReqDTO().getIsBPL().equals(MainetConstants.BLANK))) {
			...
			} else if (model.getReqDTO().getYearOfIssue() == null
					|| (model.getReqDTO().getYearOfIssue().equals(MainetConstants.BLANK))) {
		...
		/*
		 * WorkflowMas mas =
		 * iWorkflowTyepResolverService.resolveServiceWorkflowType(model.getOrgId(),
		 * model.getReqDTO().getDeptId(), model.getServiceId(),
		 * model.getReqDTO().getApplicantDTO().getDwzid1(),
		 * model.getReqDTO().getApplicantDTO().getDwzid1(),
		 * model.getReqDTO().getApplicantDTO().getDwzid1(),
		 * model.getReqDTO().getApplicantDTO().getDwzid1(),
		 * model.getReqDTO().getApplicantDTO().getDwzid1()); if (mas == null ||
		 * mas.equals(MainetConstants.BLANK)) {
		 * entityValidationContext.addOptionConstraint(session.getMessage(
		 * "rti.validation.workflow")); }
		 */
		...
		if (model.getReqDTO().getApplReferenceMode() != 0) {
			LookUp look = CommonMasterUtility.getNonHierarchicalLookUpObject(model.getReqDTO().getApplReferenceMode());

			if (look != null && !StringUtils.isEmpty(look.getLookUpCode())
					&& (look.getLookUpCode().equals("P") || look.getLookUpCode().equals(MainetConstants.FlagS))) {
				if (!CollectionUtils.isEmpty(model.getFetchStampDoc())) {
					for (final DocumentDetailsVO doc : model.getFetchStampDoc()) {
		...
		if (((model.getUploadFileList() != null) && !model.getUploadFileList().isEmpty())&& !Utility.isEnvPrefixAvailable(UserSession.getCurrent().getOrganisation(), MainetConstants.APP_NAME.DSCL)) {
		...
		}//13009 for validation when document is empty or null 
		else {
		...
		Boolean mas = false;
		try {
			mas = iRtiDetailsService.resolveServiceWorkflowType(model.getReqDTO());
		} catch (Exception e) {
