...
<div class="clearfix" id="home_content">
	<div class="col-xs-12">
		...
		<div class="row">
			...
			<p>We are redirecting to ISGPay in few secs. <br>
	 		...
	 		<form:form 	action="${command.pgUrl}" id="pay" name="pay" method="post">
				<input type="hidden" name="MerchantId" value="${command.merchantId}" />
				<input type="hidden" name="TerminalId" value="${command.terminalId}"/>
				<input type="hidden" name="BankId" value="${command.bankIdStr}" />
			    <input type="hidden" name="EncData" value="${command.payRequestMsg}" />
				 </form:form>	
			</div>
		</div>
	</div>
