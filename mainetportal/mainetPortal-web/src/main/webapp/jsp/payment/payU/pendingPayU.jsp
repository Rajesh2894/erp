...
<c:set var="myMap" value="${command}"/>	
  ...
  <div class="content animated slideInDown"> 
      <div class="widget invoice" id="bill">
        <div class="widget-content padding">
          ...
          <c:choose>
          <c:when test="${myMap.get('status') eq 'success'}">
          <div class="alert alert-success padding-15 text-center">
        	${myMap.get('response_msg')}:<strong>${myMap.get('udf2')}</strong>
        </div>
         </c:when>
         <c:otherwise>
        <div class="alert alert-danger padding-15 text-center">
        	${myMap.get('response_msg')}
        </div>
        </c:otherwise>
         </c:choose>
  ...
  <tr>
    <th><spring:message code="" text="Transaction Failure Reason"/></th>
