...
$('.form-control').bind("cut copy paste",function(e) {
	...
	function set() {
		...
		$('#emplType option[value="' + empTypeValue + '"]').prop('selected',
				true);
	}
		...
		<div class="login-panel">
			<div class="widget margin-bottom-0">
				<div class="widget-header">
					<h2>
						<strong><spring:message
								code="eip.citizen.resetPassword.validateMobile.FormHeader"
								text="Citizen Reset Password" /> </strong>
					</h2>
				</div>
				<div class="widget-content padding">
					...
					<form:form id="agencyValidateMobileForm"
						name="agencyValidateMobileForm" method="POST" autocomplete="off">
						<spring:message code="eip.select.type" var="selectLabelLang" />
						<div class="form-group">
							<label for="type"><i class="fa fa-slideshare"></i>
 		...
 		<form:select path="agencyEmployee.emplType" class="form-control" id="emplType" name="agencyEmployee.emplType">
			...
			<c:forEach items="${command.getLevelData(baseLookupCode)}" var="lookUp">
				<form:option value="${lookUp.lookUpId}" code="${lookUp.lookUpCode}">${lookUp.lookUpDesc}</form:option>
							...
							<form:hidden path="empType" id="hiddenEmpType"
								value="${command.empType}" />
							<!--  <span class="mand">*</span>  -->
						</div>
						<div class="form-group">
							<label><i class="fa fa-user"></i> <spring:message
									code="eip.citizen.resetPassword.validateMobile.MobileNo"
									text="Mobile No." /></label>

							<form:input id="mobileNumber"
								cssClass=" form-control mandClassColor hasMobileNo12"
								path="mobileNumber" onkeypress="return tryStepI(event)"
								maxlength="12" />
						...
						<div class="row">
							<div class="col-lg-6 col-md-6 col-xs-6">
								<input type="button" class="btn btn-success btn-block"
									onclick="getAgencyResetPassStepII();"
