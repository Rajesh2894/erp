<%@ taglib prefix="spring" uri="http://www.springframework.org/tags"%>
<%@ taglib prefix="apptags" tagdir="/WEB-INF/tags"%>


 <ol class="breadcrumb">
      <li  class="breadcrumb-item"><a href="CitizenHome.html"><strong class="fa fa-home"></strong> Home </a></li>
      <li class="breadcrumb-item active"><spring:message code="eip.citizen.aboutUs.title"/></li>
</ol>
     
    
    
    
 <div class="container-fluid dashboard-page ">
 	<div class="col-sm-12" id="nischay">
                         <h3><spring:message code="top.aboutus"/></h3>
          	 			<div class="widget-content padding form-horizontal">
          
          <div class="panel-group accordion-toggle" id="accordion_single_collapse">
            <div class="panel panel-default">
              <div class="panel-heading">
               <h4 class="panel-title"><spring:message code="top.aboutus"/></h4>
              </div>
             
              <div class="panel-collapse collapse in">
              <div class="panel-body">
              		<p>${command.aboutUsDescFirstPara}</p>
            	</div>
            	</div>
		</div>
		
		 <div class="panel panel-default">
              <div class="panel-heading">
               <h4 class="panel-title"><spring:message code="eip.citizen.aboutUs.vision" text=""/></h4>
              </div>
              <div class="panel-collapse collapse in">
              <div class="panel-body">
 					<p>${command.aboutUsDescSecondPara}</p>
            	</div>
	
	
		</div>
		</div>
		</div>
		</div>
          
              
              
            	</div>
          
          </div>	
