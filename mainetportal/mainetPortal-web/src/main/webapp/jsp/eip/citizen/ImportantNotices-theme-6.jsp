...
<ol class="breadcrumb">
	...
	<li class="breadcrumb-item active"><spring:message code="theme6.portal.important.notices" text="Important Notices" /></li>
</ol>
				...
				<table class="table table-bordered table-striped dataTableNormal">
					...
					<tbody>
						...
						<fmt:formatDate value="${lookUp.updatedDate}" pattern="yyyy-MM-dd HH:mm:ss" var="uDate"/>
							<%-- <c:if test="${lookUp.isUsefullLink ne 'P' && lookUp.isUsefullLink ne 'Q' &&  lookUp.isUsefullLink ne 'T'  &&  iDate le myDate &&  myDate le vDate}"> --%>
	                        <c:if test="${lookUp.isUsefullLink eq 'I' &&  iDate le myDate &&  myDate le vDate}">
