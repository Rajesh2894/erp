...
.card-box  .parent-tab>li.active>a, .nav-tabs>li.active>a:focus,
	.nav-tabs>li.active>a:hover {
	border-width: 0;
	background-color: #f2f1f1;
}

.card-box  .parent-tab>li>a {
	border: none;
	color: #373737 !important;
	font-size: 1.1em;
}

.card-box  .parent-tab>li.active>a::after, .parent-tab>li:hover>a::after
	{
	transform: scale(1);
	height: 4px;
}
	 ...
	 var secondAppealTable = $('#secondAppealTable').DataTable({
	    ...
	    $('#loader').css("display", "block"); 
	 	$.each(appList,function(i){
			...
			var uniqId = (app.appId != null)?app.appId:((app.refId != null)?app.refId:'');
			var uniqIdToDispaly = (app.appId != null && app.refId != null)?app.refId:uniqId;
			
			if(app.status=='CLOSED' ){
				closedCount++;
			
              //Defect #127174 changes for showing feedback button in care closed application
              var feedback="" ,reopenBT=""; 
				if(app.smShortdesc=="CARE"){
					var feedBackFlag=isFeedBackOnce(app.appId);
				// defect - 130933
					if(feedBackFlag=="true" || feedBackFlag){
						feedback= "<a class='btn btn-darkblue-2 btn-sm' title='Open Feedback form' onclick=\"viewCareFeedBackAlert()\">'<i class='fa fa-commenting '></i></a> ";
					}
					else{
						feedback= "<a class='btn btn-darkblue-2 btn-sm' title='Open Feedback form'  onclick=\"viewCareFeedBack('"+app.appId+"','"+app.status+"')\"><i class='fa fa-commenting '></i></a> ";
					}
					
					if($('#env').val()!='KDMC'){
 					reopenBT= "<a class='btn btn-darkblue-2 btn-sm' title='Open Reopen form'  onclick=\"viewCareReopen('"+app.appId+"','"+app.refId+"')\"><i class='fa fa-repeat '></i></a> ";
					}
				}
				 if((app.dpDeptcode=='RTS' || app.dpDeptcode=='BND') && app.docName!=null ){
				       closedList.push([app.appId,'<span style="display:none">'+moment(app.appDate,'DD/MM/YYYY HH:mm A').format("YYYYMMDD HHmm")+'</span>'+app.appDate,app.deptName,(($('#langId').val()==1)?app.serviceName : app.smServiceNameMar),app.statusDesc,"<div class='text-center'>"+
					    ...
					    "<a class='btn btn-danger  btn-sm' title='View Applicaiton History'  onclick=\"dashboardViewHistory('"+app.appId+"','"+app.smShortdesc+"','"+app.orgId+"')\"><i class='fa fa-history''></i></a> " + 
					    "<a class='btn btn-primary  btn-sm' title='Download'  onclick=\"dashboardDownload('"+app.docName+"','"+app.docPath+"','"+app.filePath+"')\"><i class='fa fa-download''></i></a> " + 
					    "</div>"]);
				      }
				   else {
				closedList.push([uniqIdToDispaly,'<span style="display:none">'+moment(app.appDate,'DD/MM/YYYY HH:mm A').format("YYYYMMDD HHmm")+'</span>'+app.appDate,app.deptName,(($('#langId').val()==1)?app.serviceName : app.smServiceNameMar),app.statusDesc,"<div class='text-center'>"+
					 "<a class='btn btn-blue-2 btn-sm'  title='View Application' onclick=\"dashboardView('"+uniqIdToDispaly+"','"+app.smShortdesc+"','"+app.status+"')\" ><i class='fa fa-eye' aria-hidden='true'></i></a> " +
		             "<a class='btn btn-danger  btn-sm' title='View Application History'  onclick=\"dashboardViewHistory('"+app.appId+"','"+app.refId+"','"+app.smShortdesc+"','"+app.orgId+"')\"><i class='fa fa-history''></i></a> " + feedback
		             +reopenBT+
					  "</div>"]);
			   }
			}
           //Defect #130191 for showing application count
			 document.getElementById('closedCount').innerHTML = closedCount;
			 if(app.status=='PENDING' && app.lastDecision=='HOLD'){
				 
				holdCount++;
				 holdList.push([uniqIdToDispaly,'<span style="display:none">'+moment(app.appDate,'DD/MM/YYYY HH:mm A').format("YYYYMMDD HHmm")+'</span>'+app.appDate,app.deptName,(($('#langId').val()==1)?app.serviceName : app.smServiceNameMar),app.statusDesc,"<div class='text-center'>"+
					"<a class='btn btn-warning  btn-sm ' title='ReSubmit Application'  onclick=\"callServiceHold('"+uniqIdToDispaly+"','"+ app.serviceEventUrl +"')\"><i class='fa fa-paper-plane-o''></i></a> "+
			        "<a class='btn btn-danger  btn-sm ' title='View Application History'  onclick=\"dashboardViewHistory('"+app.appId+"','"+app.refId+"','"+app.smShortdesc+"','"+app.orgId+"')\"><i class='fa fa-history''></i></a> "+ 
					  "</div>"]);
					  
			}
 //Defect #130191 for showing application count 
			 document.getElementById('holdCount').innerHTML = holdCount;
		if(app.status=='PENDING' && app.lastDecision!='HOLD'){
				
				pendingCount++;         
				pendingList.push([uniqIdToDispaly,'<span style="display:none">'+moment(app.appDate,'DD/MM/YYYY HH:mm A').format("YYYYMMDD HHmm")+'</span>'+app.appDate,app.deptName,(($('#langId').val()==1)?app.serviceName : app.smServiceNameMar),app.statusDesc,"<div class='text-center'>"+
					 "<a class='btn btn-blue-2 btn-sm '  title='View Application' onclick=\"dashboardView('"+uniqIdToDispaly+"','"+app.smShortdesc+"','"+app.status+"')\" ><i class='fa fa-eye' aria-hidden='true'></i></a> " +
		             "<a class='btn btn-danger  btn-sm ' title='View Application History'  onclick=\"dashboardViewHistory('"+app.appId+"','"+app.refId+"','"+app.smShortdesc+"','"+app.orgId+"')\"><i class='fa fa-history''></i></a> "  + 
					  "</div>"]);
			}
		...
		document.getElementById('pendingCount').innerHTML = pendingCount;
			...
			if(app.status== "EXPIRED"){
				expireCount++;
				expireList.push([uniqIdToDispaly,'<span style="display:none">'+moment(app.appDate,'DD/MM/YYYY HH:mm A').format("YYYYMMDD HHmm")+'</span>'+app.appDate,app.deptName,(($('#langId').val()==1)?app.serviceName : app.smServiceNameMar),app.statusDesc,"<div class='text-center'>"+
					 "<a class='btn btn-blue-2 btn-sm '  title='View Application' onclick=\"dashboardView('"+uniqIdToDispaly+"','"+app.smShortdesc+"','"+app.orgId+"')\" ><i class='fa fa-eye' aria-hidden='true'></i></a> " +
		             "<a class='btn btn-danger  btn-sm ' title='View Application History'  onclick=\"dashboardViewHistory('"+app.appId+"','"+app.refId+"','"+app.smShortdesc+"','"+app.orgId+"')\"><i class='fa fa-history''></i></a> "  + 
					  "</div>"]);
			}
              //Defect #130191 for showing application count
			document.getElementById('expireCount').innerHTML = expireCount;
		/* 	if(app.serviceEventName== $("#loiPayment").val()){
			...
			if(app.status=="DRAFT" ){
				draftCount++;
				draftList.push([uniqIdToDispaly,'<span style="display:none">'+moment(app.appDate,'DD/MM/YYYY HH:mm A').format("YYYYMMDD HHmm")+'</span>'+app.appDate,app.deptName,(($('#langId').val()==1)?app.serviceName : app.smServiceNameMar),app.statusDesc,"<div class='text-center'>"+
					"<a class='btn btn-blue-2 btn-sm'  title='View Application' onclick=\"dashboardView('"+uniqIdToDispaly+"','"+app.smShortdesc+"','"+app.status+"')\" ><i class='fa fa-eye' aria-hidden='true'></i></a> " +
		             "<a class='btn btn-danger  btn-sm' title='View Application History'  onclick=\"dashboardViewHistory('"+app.appId+"','"+app.refId+"','"+app.smShortdesc+"','"+app.orgId+"')\"><i class='fa fa-history''></i></a> " + 
					  "</div>"]);
			}
              //Defect #130191 for showing application count
			document.getElementById('draftCount').innerHTML = draftCount;
		...
		pendingTable.rows.add(pendingList);
	 	...
	 	$.each(payPendingList,function(i){
			var app = payPendingList[i];
				payPendCount++;
				payPendList.push([app.refId,'<span style="display:none">'+moment(app.appDate,'DD/MM/YYYY HH:mm A').format("YYYYMMDD HHmm")+'</span>'+app.appDate,app.deptName,app.deptNameMar,(($('#langId').val()==1)?app.serviceName : app.smServiceNameMar),app.status,"<div class='text-center'>"+
         ...
         //Defect #130191 for showing application count
	 	document.getElementById('payPendCount').innerHTML = payPendingList.length;
	 	...
	 	$.each(loiPaymentList,function(i){
			...
			loiPendingCount++;
			loiPaymntList.push([app.loiApplicationId,(($('#langId').val()==1)?app.serviceName : app.serviceNameMar),app.loiNo,app.loiRefId,app.loiAmount,app.loiPaid ,"<div class='text-center'>"+
         ...
         //Defect #130191 for showing application count
	 	document.getElementById('loiPendingCount').innerHTML = loiPaymentList.length;
	 	...
	 	$.each(rtsFirstAppealList,function(i){
			...
			var uniqIdToDispaly = (app.appId != null && app.refId != null)?app.refId:uniqId;
	 		firstAppealCount++;
		 	firstAppealList.push([uniqIdToDispaly,'<span style="display:none">'+moment(app.appDate,'DD/MM/YYYY HH:mm A').format("YYYYMMDD HHmm")+'</span>'+app.appDate,app.deptName,(($('#langId').val()==1)?app.serviceName : app.smServiceNameMar),app.status,"<div class='text-center'>"+
					 "<a class='btn btn-blue-2 btn-sm'  title='View Application' onclick=\"dashboardViewFirstRTS('"+uniqIdToDispaly+"','"+app.dpDeptcode+"','"+app.status+"')\" ><i class='fa fa-eye' aria-hidden='true'></i></a> " +
		             "<a class='btn btn-danger  btn-sm' title='View Application History'  onclick=\"dashboardViewHistory('"+app.appId+"','"+app.refId+"','"+app.smShortdesc+"','"+app.orgId+"')\"><i class='fa fa-history''></i></a> " + 
					  "</div>"]);
		 	
	 	});
           //Defect #130191 for showing application count
	 	document.getElementById('firstAppealCount').innerHTML = rtsFirstAppealList.length;
	 	...
	 	$.each(rtsSecondAppealList,function(i){
			...
			secondAppealCount++;
			secondAppealList.push([app.appId,app.refId,app.serviceName,'<span style="display:none">'+moment(app.appDate,'DD/MM/YYYY HH:mm A').format("YYYYMMDD HHmm")+'</span>'+app.appDate,app.status,"<div class='text-center'>"+
        ...
        //Defect #130191 for showing application count
	 	document.getElementById('secondAppealCount').innerHTML = secondAppealList.length;
	    ...
	    billingTable.rows().remove().draw();
	 	...
	 	$.each(billList,function(i){
			var bill = billList[i];
				closedCount++;
				billingList.push([bill.proertyNo,'<span style="display:none">'+moment(bill.appDate,'DD/MM/YYYY HH:mm A').format("YYYYMMDD HHmm")+'</span>'+bill.appDate,bill.deptName,"<div class='text-center'>"+
         ...
         //Defect #130191 for showing application count
	 	document.getElementById('billPaymentCount').innerHTML = billList.length;
		...
		console.log(waterBillingList.length);
	 	...
	 	$("#externalAppeal").on('click', function() {
	 		  //open the external form
	 		 let viewFormDetails =__doAjaxRequest('RTSExternalAppeal.html?openExternalAppealForm', 'POST', {}, false);
...
//Defect #127174 changes for showing feedback button in care closed application
 function viewCareFeedBack(appId,appStatus)
 {
 	var data = {"appId":appId,
 			"appStatus":appStatus}; 
 	
 	var viewFormDetails=__doAjaxRequest('grievance.html?viewCareFeedBack', 'POST',data,false);
 	

 	$(formDivName).html(viewFormDetails);
 }
 
 function  viewCareFeedBackAlert(){
	 alert(getLocalMessage("citizen.dashboard.feedBackAlert"));
 }
//Defect #127174 changes for showing feedback button in care closed application
 function isFeedBackOnce(appId)
 {
 	var data = {"appId":appId}; 
 	
 	var feedFlag=__doAjaxRequest('grievance.html?isFeedBackOnce', 'POST',data,false);
 	
 	return feedFlag;

 }
 function viewCareReopen(appId,compNo)
 {
 	let data = {"appId":appId,"compNo":compNo}
 	
 	let viewFormDetails=__doAjaxRequest('grievance.html?viewCareReopen', 'POST',data,false);
...
$(function () {
   ...
   Highcharts.setOptions({
    ...
    $('#container1').highcharts({
        ...
        title: {
            enabled: true,
            text: getLocalMessage("citizen.application.service.apps"),
            verticalAlign: 'top',
            style: {
                color: '#000',
		...
		xAxis: {
		     categories: [
		    	 "<spring:message code="citizen.application.pending.applications" text="Pending Applications"/>",
		    	 "<spring:message code="citizen.application.closed.applications" text="Closed Applications"/>",
		    	 "<spring:message code="citizen.application.hold.applications" text="Hold Applications"/>",
		    	 "<spring:message code="citizen.application.expired.applications" text="Expired Application"/>",
		    	 "<spring:message code="citizen.application.payment.pending.applications" text="Payment Pending Applications"/>",
		    	 "<spring:message code="citizen.application.first.appeal" text="First Appeal"/>",
		    	 "<spring:message code="citizen.application.second.appeal" text="Second Appeal"/>"
		    	],
		   },	  
		   series: [{
		       name: '#',
...
<div id="dashBoardDiv" class="cit-dash">
    ...
    <c:set var = "langId" value ="${userSession.languageId eq 1}"/>
    <input type ="hidden" value = "${userSession.getCurrent().getOrganisation().getOrgShortNm()}" id ="env"/>
    <input type ="hidden" value = "${userSession.languageId }" id ="langId"/>
	<ul class="breadcrumb" tabindex="0">
		<li tabindex="0"><i class="fa fa-home"></i></li>
		<li tabindex="0"><spring:message code="citizen.link.name"></spring:message></li>
		<li class="active" tabindex="0"><spring:message
	...
	<div class="content">
		...
		<div class="animated">
			<div class="sidebar-menu-btn">
				<a class="btn menu-btn menuBtn" href="#" tabindex="0"><spring:message code="CitizenServices" text="CitizenServices"></spring:message></a>
				<a class="btn menu-btn-close menuBtnClose" tabindex="0"><spring:message code="bt.close" text="Close"></spring:message></a>
			</div>
					...
					<ul class="nav nav-tabs parent-tab " role="tablist">
						<li role="presentation" class="active col-sm-4 text-center"><a
							href="#home" aria-controls="home" role="tab" data-toggle="tab">
								<spring:message code="citizen.application.status.service.applications" text="Status of Service Applications" />
						</a></li>
						<li role="presentation" class="col-sm-4 text-center"><a
							href="#payment" aria-controls="home" role="tab" data-toggle="tab">
								<spring:message code="citizen.application.status.payments" text="Status of Payments" />
						</a></li>
						<li role="presentation" class="col-sm-4 text-center"><a
						...
						<div role="tabpanel" class="tab-pane active" id="home">
							...
							<ul id="demo1" class="nav nav-tabs ">
								<li class="active "><a href="#Inprocess" class="pending"
									data-toggle="tab"><spring:message
											code="citizen.application.applicationComp_Reject" /> <span id="pendingCount" class="count-indicator"></span></a></li>
								<li><a href="#closed" data-toggle="tab"><spring:message
											code="citizen.application.applicationcomlete"
											text="Applications Closed" /> <span id="closedCount" class="count-indicator"></span></a></li>
								<li><a href="#Pending" data-toggle="tab"><spring:message
											code="citizen.application.applicationhold"
											text="Application Hold" /> <span id="holdCount" class="count-indicator"></span></a></li>
								<li><a href="#Expired" data-toggle="tab"><spring:message
											code="citizen.application.applicationexpired"
											text="Application Expired" /><span id="expireCount" class="count-indicator"></span></a></li>
								<li><a href="#draft" data-toggle="tab"><spring:message
											code="citizen.application.applicationdraft"
											text="Application Draft" /> <span id="draftCount" class="count-indicator"></span></a></li>
								<li><a href="#firstAppeal" data-toggle="tab"><spring:message
											code="citizen.application.first.appeal" text="First Appeal" /> <span id="firstAppealCount" class="count-indicator"></span></a></li>
								<li><a href="#secondAppeal" data-toggle="tab"><spring:message
											code="citizen.application.second.appeal" text="Second Appeal" /> <span id="secondAppealCount" class="count-indicator"></span></a></li>
								<li><a href="#" id="externalAppeal" data-toggle="tab"><spring:message
											code="citizen.application.external.appeal" text="External Appeal" /> <span id="externalCount" class="count-indicator"></span></a></li>
							</ul>
							<div class="tab-content tab-boxed">


								<div class="tab-pane fade active in" id="Inprocess">
										<div id="loader" class="spinner" align="center">Loading Data....</div>
									<table
										class="table-responsive table table-striped table-bordered dataTable"
										id="pendingTable">
										<thead>
											<tr>
						...
						<div role="tabpanel" class="tab-pane" id="payment">
							...
							<ul id="demo2" class=" nav nav-tabs ">
								<li class="active "><a href="#LoiPayment" data-toggle="tab"><spring:message
											code="citizen.application.loi.to.be.paid" text="LOI To Be Paid" /> <span id="loiPendingCount" class="count-indicator"></span></a></li>
								<li><a href="#OnlineTransfail" data-toggle="tab"><spring:message
											code="citizen.application.applicationpayment"
											text="Application Payment Pending" /> <span id="payPendCount" class="count-indicator"></span></a></li>
						...
						<div role="tabpanel" class="tab-pane" id="biil-status">
							...
							<ul id="demo3" class=" nav nav-tabs ">
								<li class="active "><a href="#Property" data-toggle="tab"><spring:message
											code="citizen.application.property" text="Property" /> <span id="billPaymentCount" class="count-indicator"></span></a></li>
								<li><a href="#Water" data-toggle="tab"><spring:message
											code="citizen.application.water" text="Water" /> <span id="waterBillPaymentCount" class="count-indicator"></span></a></li>
							</ul>
							<div class="tab-content tab-boxed">
			...
			<%-- <c:if
				...
				<div class="container">
					<div class="row">
						<div
							class="col-lg-4 col-lg-offset-8 col-md-5 col-md-offset-7 col-sm-6 col-sm-offset-6 col-xs-11 col-xs-offset-1"
							id="jumpTask">
							<select id="selectedTask" data-content=""
								class="form-control chosen-select-no-results">
								<option value="0">Search Task</option>
								<c:forEach var="masters"
									items="${menuRoleEntitlement.parentList}">
			...
			<h2 id="dashmenu">
				<span class="hidden"><spring:message code="CitizenServices"
						text="CitizenServices"></spring:message></span>
			</h2>
			<c:set var="langId" value="${userSession.getCurrent().getLanguageId() }"/>
			<div class="col-xs-12 col-sm-3 col-md-3 col-lg-3 padding-right-0 sidebar-main">
				<div id="sidebar-menu" tabindex="0">
					<ul id="nav" class="has_sub" tabindex="0">
						<li id="0" style="display: none"><a>Search Task</a></li>
						<c:forEach var="masters" items="${menuRoleEntitlement.parentList}">
							<li class="has_sub" id="${masters.entitle.smfid}"><a
								role="link" title="${(langId eq 1) ? masters.entitle.smfname_eng : masters.entitle.smfname_mar}"
								id="go${masters.entitle.smfid}"
								name="go${masters.entitle.smfid}"
								href="${masters.entitle.smfaction}"
								tabindex="0"
								onclick="dodajAktywne(this);
								
								<%-- breadcrumb('${masters.entitle.smfname_eng}+${data.entitle.smfname_eng}',${masters.entitle.smfid});"><span>${masters.entitle.smfname_eng}</span> --%>
							breadcrumbMultiLang('${masters.entitle.smfname_eng}+${data.entitle.smfname_eng}','${masters.entitle.smfname_mar}+${data.entitle.smfname_mar}',${masters.entitle.smfid});"><span>${(langId eq 1)  ? masters.entitle.smfname_eng : masters.entitle.smfname_mar}</span>
							</a>
							...
							<ul class="scroll-menu">
								<c:forEach items="${menuRoleEntitlement.childList}" var="data">
									<c:if test="${masters.entitle.smfid eq  data.parentId}">
									<c:set var="action0" value="${data.entitle.smfaction}" />
									<li class="navigation" id="${data.entitle.smfid}"><a
										tabindex="0"
										role="link" title="${(langId eq 1) ? data.entitle.smfname_eng : data.entitle.smfname_mar}"
										<c:choose> <c:when test="${fn:containsIgnoreCase(action0 , '.html')}">onclick="openRelatedForm('${action0}','this','${data.entitle.smfdescription}');
										<%-- breadcrumb('${masters.entitle.smfname_eng}+${data.entitle.smfname_eng}',${masters.entitle.smfid});" href="#"</c:when><c:otherwise> href="${action0}" </c:otherwise></c:choose>>${data.entitle.smfname_eng} </a> --%>
									breadcrumbMultiLang('${masters.entitle.smfname_eng}+${data.entitle.smfname_eng}','${masters.entitle.smfname_mar}+${data.entitle.smfname_mar}',${masters.entitle.smfid});" href="#"</c:when><c:otherwise> href="${action0}" </c:otherwise></c:choose>>${(langId eq 1)  ?  data.entitle.smfname_eng : data.entitle.smfname_mar} </a>
										...
										<c:forEach items="${menuRoleEntitlement.childList}"
											...
											<li class="folder " id="${data1.entitle.smfid}"><a
												tabindex="0"
												title="${data1.entitle.smfname_eng}"
												<c:choose><c:when test="${fn:containsIgnoreCase(action1 , '.html')}">onclick="openRelatedForm('${action1}');breadcrumb('${masters.entitle.smfname_eng}+${data.entitle.smfname_eng}+${data1.entitle.smfname_eng}',${masters.entitle.smfid});" href="#"</c:when><c:otherwise>href="${action1}"</c:otherwise></c:choose>>${(langId eq 1) ? data1.entitle.smfname_eng : data1.entitle.smfname_mar}</a>
												<ul>
												<c:forEach items="${menuRoleEntitlement.childList}"
													var="data2">
													<c:if test="${data1.entitle.smfid eq  data2.parentId}">
													...
													<li class="folder" id="${data2.entitle.smfid}"><a
														tabindex="0"
														role="link" title="${(langId eq 1) ? data2.entitle.smfname_eng : data2.entitle.smfname_mar}"
														<c:choose><c:when test="${fn:containsIgnoreCase(action2 , '.html')}">onclick="openRelatedForm('${action2}');
														<%-- breadcrumb('${masters.entitle.smfname_eng}+${data.entitle.smfname_eng}+${data1.entitle.smfname_eng}+${data2.entitle.smfname_eng}',${masters.entitle.smfid});" href="#"</c:when><c:otherwise>href="${action2}"</c:otherwise></c:choose>>${data2.entitle.smfname_eng}</a> --%>
														breadcrumbMultiLang('${masters.entitle.smfname_eng}+${data.entitle.smfname_eng}+${data1.entitle.smfname_eng}+${data2.entitle.smfname_eng}','${masters.entitle.smfname_mar}+${data.entitle.smfname_mar}+${data1.entitle.smfname_mar}+${data2.entitle.smfname_mar}',${masters.entitle.smfid});" href="#"</c:when><c:otherwise>href="${action2}"</c:otherwise></c:choose>>${(langId eq 1) ?  data2.entitle.smfname_eng :  data2.entitle.smfname_mar}</a>
														<ul>
														<c:forEach items="${menuRoleEntitlement.childList}"
															var="data3">
															<c:if test="${data2.entitle.smfid eq  data3.parentId}">
															<c:set var="action3" value="${data3.entitle.smfaction}" />
																...
																tabindex="0"
																	role="link" title="${(langId eq 1) ? data3.entitle.smfname_eng : data3.entitle.smfname_mar}"
																	<c:choose><c:when test="${fn:containsIgnoreCase(action3 , '.html')}">onclick="openRelatedForm('${action3}');
																	<%-- breadcrumb('${masters.entitle.smfname_eng}+${data.entitle.smfname_eng}+${data1.entitle.smfname_eng}+${data2.entitle.smfname_eng}+${data3.entitle.smfname_eng}',${masters.entitle.smfid});" href="#"</c:when><c:otherwise>href="${action3}"</c:otherwise></c:choose>>${data3.entitle.smfname_eng}</a></li> --%>
															    breadcrumbMultiLang('${masters.entitle.smfname_eng}+${data.entitle.smfname_eng}+${data1.entitle.smfname_eng}+${data2.entitle.smfname_eng}+${data3.entitle.smfname_eng}','${masters.entitle.smfname_mar}+${data.entitle.smfname_mar}+${data1.entitle.smfname_mar}+${data2.entitle.smfname_mar}+${data3.entitle.smfname_mar}',${masters.entitle.smfid});" href="#"</c:when><c:otherwise>href="${action3}"</c:otherwise></c:choose>>${(langId eq 1) ? data3.entitle.smfname_eng :  data3.entitle.smfname_mar}</a></li>
...
$(window).on('load', function(){
   ...
   $("#selectedTask").on("change",function(){
	   ...
	   $($("li[id="+search+"]").parentsUntil("ul#nav")).each(function() {if($(this).is("ul")){$(this).css("display","block")}});
 	   $('html, body').animate({
 		   scrollTop: $("li[id="+search+"] > a").addClass("flashi").offset().top-100}, 'slow'); 	   
   }) 
  });
</script>