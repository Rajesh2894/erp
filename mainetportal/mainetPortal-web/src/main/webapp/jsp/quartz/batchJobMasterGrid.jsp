...
<form:form action="QuartzSchedulerMaster.html"	name="frmQuartzSchedulerMaster" id="frmQuartzSchedulerMaster" cssClass="form-horizontal">
		...
		<div class="error-div" style="display: none" id="errorDivId"></div>
...
<!-- Start Each Section -->
				...
				<apptags:selectField items="${command.departmentList}" selectOptionLabelCode="quartz.form.fieldName.selectDep"
				 fieldPath="departmentForQuartz" cssClass="input2 hasNameClass form-control" hasId="true"/>
				</div> 
				
				<div class="col-sm-6">
				<input type="submit" class="btn btn-info" onclick="return openBatchJobEditForm()"	value="<spring:message code="quartz.form.button.addJob" />" />
				</div>
				<form:hidden path="" value="${command.departmentList}" id="deptListId"/>
			</div>
	...
	<!-- End Each Section -->
		...
		<input type="button" class="btn btn-success" onclick="return searchAvailableJobsByDepartment()"	value="<spring:message code="quartz.form.search" />" />
		<apptags:resetButton/>
		</div>
...
<div class="panel panel-default">
        <div class="panel-heading">
            <h4> <spring:message code="quartz.bredCrum.header.batchJobDetails" /> </h4>
        </div>
		...
		<div class="form-div">			
			<apptags:jQgrid id="quartzSchedulerMasterEntryForm"
				...
				hasViewDet="false" hasDelete="false" height="100" hasEdit="true"
				caption="quartz.bredCrum.header.batchJobDetails" loadonce="true" />
		</div>
		</div>
            </div>
        </div>
