...
function getPropertyDetails(index) {
	...
	if (errorList.length == 0) {
		...
		var URL = 'DuplicateBillForm.html?getPropertyDetailsByFlatNo';
...
function getFlatDetails(rowNo, dec) {
	...
	if (propNo != 0) {
		...
		var URL = 'DuplicateBillForm.html?getBillingMethod';
		...
		if (ajaxResponse) {
			if (ajaxResponse.length != 0) {
				...
				if(flatNumber != '' && flatNumber != undefined){
					$("#flatNo" + (rowNo)).append("<option value='" + flatNumber + "'>"
							+ flatNumber + "</option>");
				}else{
...
function continueForPayment(element) {
	if ($("#onlineOfflinePay").val() == 'N'
		...
		window.open('DuplicateBillForm.html?PrintReport', '_blank');
	} else if ($("#onlineOfflinePay").val() == 'Y') {
		closebox("", "DuplicateBillForm.html?redirectToPay");
	}
}
