...
function saveData(element){
	...
	var response = __doAjaxRequest(url+'?showCheckList', 'POST',requestData, false, '', 'html');
...
function applicationFormValidation(errorList) {
	...
	if(schemenameId == "0" || schemenameId == undefined
			|| schemenameId == '')
		{
	...
	if(ageason == "0" || ageason == undefined
			|| ageason == '')
		{
		errorList.push(getLocalMessage('social.sec.ageason.req'));
		}
	
	if(genderId == "0" || genderId == undefined
			|| genderId == '')
		{
	...
	if(pinCode == "0" || pinCode == undefined
			|| pinCode == '')
		{
	...
	if (aadharCard == "0" || aadharCard == undefined
			|| aadharCard == '') {
		errorList.push(getLocalMessage('Aadhar Number required'));
	}
	
	if(mobNum == "0" || mobNum == undefined
			|| mobNum == '')
		{
	...
	if(typeofDisId == "0" || typeofDisId == undefined
			|| typeofDisId == '')
		{
		errorList.push(getLocalMessage('social.sec.typeofdis.req'));
		}
	
	let typeofdisvar=$('#applicationformdto\\.typeofDisId option:selected').attr('code'); 
	if(typeofdisvar!="NA")
		{
	...
	if(bplfamily == "0" || bplfamily == undefined
			|| bplfamily == '')
		{
	...
	if(banknameId == "0" || banknameId == undefined
			|| banknameId == '')
		{
	...
	if(nameofFather == "0" || nameofFather == undefined
			|| nameofFather == '')
		{
		errorList.push(getLocalMessage('social.sec.namefather.req'));
		}
	
	if(nameofMother == "0" || nameofMother == undefined
			|| nameofMother == '')
		{
	...
	if(detailsoffamIncomeSource == "0" || detailsoffamIncomeSource == undefined
...
function saveCheckListAppForm(element) {
	...
	var ajaxResponse = __doAjaxRequest('SchemeApplicationForm.html?saveCheckListAppdetails', 'POST', requestData, false,'',element);
	...
	if(errorsPresent.length > 0) {
		...
		$('.pagedivCheckList').html(ajaxResponse);
		prepareDateTag();
	}else {
		if ($.isPlainObject(ajaxResponse))
		{
			var message = ajaxResponse.command.message;
			showMessageOnSubmit(ajaxResponse,message,'SchemeApplicationForm.html');
		}
	}
}
var correct =true;
...
function showMessageOnSubmit(successMsg,message,redirectURL) {
	...
	var cls = getLocalMessage('social.sec.proceed.button');
