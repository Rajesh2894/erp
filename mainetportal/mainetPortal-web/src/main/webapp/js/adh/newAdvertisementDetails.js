...
function validateAdvertisingDetails(errorList){
	...
	$('.advertisingDetailsClass').each(function(i) {
		...
		if (adhTypeId1 == '' || adhTypeId1 == undefined || adhTypeId1 == '0') {
				errorList.push(getLocalMessage("Select Advertiser Type")+" " +level);
			}
		if (adhTypeId2 == '' || adhTypeId2 == undefined || adhTypeId2 == '0') {
				errorList.push(getLocalMessage("Select Advertiser Subtype" + " ")+" " +level);
			}
		if (advDetailsDesc == '' || advDetailsDesc == undefined || advDetailsDesc == '0') {
			errorList.push(getLocalMessage("Please Enter Description "+ " ")+" " +level);
		}
		if (advDetailsHeight == '' || advDetailsHeight == undefined || advDetailsHeight == '0.00') {
			errorList.push(getLocalMessage("Please Enter Height "+ " ")+" " +level);
		}
		if (advDetailsHeight == '' || advDetailsHeight == undefined || advDetailsHeight == '0.00') {
			errorList.push(getLocalMessage("Please Enter Length "+ " ")+" " +level);
		}
		if (advDetailsArea == '' || advDetailsArea == undefined || advDetailsArea == '0.00') {
			errorList.push(getLocalMessage("Please Enter Area "+ " ")+" " +level);
		}
		if (dispTypeId == '' || dispTypeId == undefined || dispTypeId == '0') {			
			errorList.push(getLocalMessage("Please Select Display Face. "+ " ")+" " +level);
		}
		if (unit == '' || unit == undefined || unit == '0') {
			if(dispTypeId == '0'){
				errorList.push(getLocalMessage("Please Enter valid number "+ " ")+" " +level);
			}else{
...
$(function() {
	...
	$("#newAdvertisingTableId").on('click', '.delButton', function() {
		...
		if ($("#newAdvertisingTableId tr").length != 2) {
			...
			var errorList = [];
			errorList.push(getLocalMessage("First row cannot be remove"));
			displayErrorsOnPage(errorList);
		}
	});
});
