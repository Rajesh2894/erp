...
$(function() {
$("#newAdvertisingTableId").on('click', '.addHoardingDet', function(e) {
	var errorList = [];
...
function reOrderAdvertisingIdSequence(advertisingDetailsClass){
	...
	$(advertisingDetailsClass).each(function(i) {
		...
		$(this).find("select:eq(0)").attr("id", "hoardingId" + i);
...
function validateHoardindDetails(errorList) {
    {
	var i = 0;
	if ($.fn.DataTable.isDataTable('#hoardingId')) {
	...
	$("#newAdvertisingTableId tbody tr")
		.each(
			function(i) {
			    ...
			    var hoardingNo = $("#hoardingId" + i).val();
