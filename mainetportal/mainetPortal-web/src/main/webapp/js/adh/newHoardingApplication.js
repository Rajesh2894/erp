...
$(document).ready(function() {
	...
	if ($("#saveMode").val() != null && $("#saveMode").val() == 'VIEW') {
		...
		$("#HoardingDetails *").attr("disabled","disabled");
	}
});
...
function saveNewAdvApplication(object) {
	var errorList = [];
	//Defect #126413
	var online = document.getElementById('onlineModeId').checked;
	/*if(online){
	...
	if (errorList.length == 0) {
		/*return saveOrUpdateForm(
				object,
				"New Hoarding Application Permission Submitted Successfully","CitizenHome.html", 'saveform');*/
		if (errorList.length == 0) {
			var theForm = '#newHoardingApplication';
			var requestData = __serializeForm(theForm);
			var object = __doAjaxRequest(
					"NewHoardingApplication.html?saveHoardingApplication",
					'POST', requestData, false, 'json');
...
function getAgencyDetails(){
	...
	if(advertiserCategoryId != '' && advertiserCategoryId != undefined){
		var requestData = {
		...
		$.each(prePopulate, function(index, value) {
			$('#agnId').append(
					$("<option></option>").attr("value", value.agencyId).text((value.agencyName)));
		});
		$('#agnId').trigger("chosen:updated");
	}
}
...
function getLicType() {
	...
	{   
		var response = doAjaxLoading('NewHoardingApplication.html?getLicenceType', requestData,'json');				
...
function showMessageForSuccess(applicationNo, servicefree) {
	...
	var no = 'No';
	var redirectUrl = 'NewHoardingApplication.html?PrintReport';
	...
	//Defect #77088-for both online and offline payment
		...
		var redirectUrl = 'NewHoardingApplication.html?redirectToPay';
		message += '<div class=\'text-center padding-bottom-10\'>'
			+ '<input class="btn btn-success" style="margin-right:10px" type=\'button\' value=\''
