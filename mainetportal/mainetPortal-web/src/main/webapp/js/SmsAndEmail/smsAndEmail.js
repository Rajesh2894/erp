...
$(function () {
    $("#grid").jqGrid({
        ...
        mtype: "GET",
        colNames: [
        	getLocalMessage("dept.master.deptLoc.deptName"),
        	getLocalMessage("master.serviceName"),
        	getLocalMessage("master.TemplateType"),
        	getLocalMessage("master.AlertType"),
        	/*getLocalMessage("master.grid.column.action")*/
        	getLocalMessage("master.view"),
        	getLocalMessage("eip.commons.bt.edit"),
        	getLocalMessage("eip.commons.bt.delete")
        ],
        colModel: [
            ...
            /*{ name: '', index: '', align: 'center !important', sortable: false, width : 25,formatter:actionFormatter,search : false}*/
            { name: '', index: '', align: 'center !important', sortable: false, width : 5,formatter:actionView,search : false},
            { name: '', index: '', align: 'center !important', sortable: false, width : 5,formatter:actionEdit,search : false},
            { name: '', index: '', align: 'center !important', sortable: false, width : 5,formatter:actionDelete,search : false}
        ],
        pager: "#pagered",
        rowNum: 30,
        rowList: [5, 10, 20, 30],
        ...
        caption: getLocalMessage("dept.master.sms.email.list")
    });
...
function actionEdit(cellvalue, options, rowObject){
	return "<a class='btn btn-warning btn-sm' title='Edit' onclick=\"editPrefix('"+rowObject.seId+"','update')\"><i class='fa fa-pencil' aria-hidden='true'></i></a> "
}
function actionDelete(cellvalue, options, rowObject){
	return "<a class='btn btn-danger btn-sm' title='Delete' onclick=\"deletePrefix('"+rowObject.seId+"')\"><i class='fa fa-trash' aria-hidden='true'></i></a> "
}
...
function showDelConfirmBox(seId){
	...
	var cls = getLocalMessage('eip.commons.yes');
	var cls2 = getLocalMessage('eip.commons.no');
	
	if(seId != 0){
		message	+='<h4 class=\"text-center text-blue-2 padding-10\">'+msg+'</h4>';
		 message	+='<div class=\'text-center padding-bottom-10\'>'+	
		'<input type=\'button\' value=\''+cls+'\'  id=\'btnNo\' class=\'btn btn-blue-2 margin-right-5 \'    '+ 
...
function fn_setService(obj,id) {
	var msg=getLocalMessage('lbl.sms.noservice');
	...
	if (deptId > 0) {
		...
		$("#serviceId").append($("<option></option>").attr("value","0").text(msg));
   ...
   if($("input[name='entity.alertType']").is(":checked")){
	  alertType = $("input[name='entity.alertType']:checked").val();  
		...
		if (result != "") {
			...
			if(!delFlag){
			errorList.push(getLocalMessage("sms.no.records"));
...
function setServiceurl() {
	...
	if (deptIDEdit > 0) {
		...
		$("#serviceId").append($("<option></option>").attr("value","").text(msg));
		$(".serviceList").append(optionsAsString);
		var serviceid =  $('#selectedServiceId').val();
		
		if(serviceid==''){
			$(".serviceList option[value='']").prop('selected',true);
		}else{
