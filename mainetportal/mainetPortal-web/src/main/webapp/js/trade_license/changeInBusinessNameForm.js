$(document).ready(function() {
	...
	//122900
	var valueEditableCheckFlag = $("#valueEditableCheckFlag").val();
	if (valueEditableCheckFlag ==  'Y'){
...
//#129512
function onPaymentYes(){debugger
$.fancybox.close();
var errorList = [];
if (errorList.length == 0) {
...
function getOtpBtn(){
	...
	if (errorList.length == 0) {
		...
		var response = __doAjaxRequest(
				'ChangeInBusinessNameForm.html?getOtpBtnShow', 'POST',
				requestData, false, 'html');
		if (response != null) {
		$(formDivName).html(response);
		}
...
function generateOtpNumber(element) {
	...
	var theForm = '#changeBusinessNameForm';
	var requestData = __serializeForm(theForm);
	var URL = 'ChangeInBusinessNameForm.html?' + 'generateOtp';
