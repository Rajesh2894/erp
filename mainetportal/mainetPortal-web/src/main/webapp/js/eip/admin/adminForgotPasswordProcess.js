...
function getAdminForgotPassStep4(){
	...
	if (errorList.length == 0) 
	{
		...
		var result = __doAjaxRequest('AdminForgotPassword.html?doResetPassword', 'post', formData, false);
 ...
 * @returns errorList
 */
function adminvalidateFormStep1() {
...
function adminvalidateFormStep2(errorList) {
	...
	if (!isEmpty('oneTimePassword')) {
		...
		var formData = $(document.getElementById(adminsetp2Form)).serialize();
		var result = __doAjaxRequest('AdminForgotPassword.html?verifyOTP', 'post', formData, false);
		$('.content-page').html(result);
	}
}
...
function getAdminLoginFrm() {
	...
	var response = __doAjaxRequest('AdminLogin.html', 'post', {}, false, 'html');
	adminLogin(response);
}
