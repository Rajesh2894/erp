...
function saveWitness(element) {
	...
	if (errorList.length > 0) {
		...
		//below code is use when online payment at application time
		/*if ($("input:radio[name='offlineDTO.onlineOfflineCheck']").filter(":checked").val() == 'Y') {
			 return saveOrUpdateForm(element,"Payment done successfully!", 'MarriageRegistration.html?redirectToPay', 'saveWitnessPage');
		}else if ($("input:radio[name='offlineDTO.onlineOfflineCheck']").filter(":checked").val() == 'N'){
			 return saveOrUpdateForm(element,"Payment done successfully!", 'MarriageRegistration.html?PrintReport', 'saveWitnessPage');
		}else{
			 return saveOrUpdateForm(element,"Payment done successfully!", 'CitizenHome.html', 'saveWitnessPage');
		}*/
...
function marriageRegAcknow(element) {
...
function validateWitnessDetails(errorList) {
	...
	let occupation = $("#witnessTabForm select[id=occupation]").find(
			":selected").val();
