...
function saveMarriage(element) {
	...
	if (errorList.length > 0) {
		...
		var errorsPresent = tempDiv.find('#validationerrordiv');
...
function validateApplicantInfo(errorList) {
    ...
    if(adharNo.length > 0){
    		if(validateAadhaar(adharNo) == false){
	...
	errorList.push(getLocalMessage('mrm.applicant.validate.pincode'));
		...
		errorList.push(getLocalMessage("mrm.validation.pincode"));
	}
