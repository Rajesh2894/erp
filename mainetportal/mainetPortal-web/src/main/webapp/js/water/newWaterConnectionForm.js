				...
				function() {
					...
					$("#billing").click(function() {
						...
						if (chk) {
							...
							$('#coBDwzid1').attr("disabled", false);
							$('#coBDwzid2').attr("disabled", false);
...
function saveWaterForm(element) {
	...
	if ($("#trmGroup1").val() == 0 && $("#trmGroup2").val() == 0) {
		$('a[href$="#waterconnectiondetails"]').removeClass('collapsed');
		$('#waterconnectiondetails').addClass('in');
		return false;
	}
	if ($('#accept').is(':checked')) {
		if ($("input:radio[name='offlineDTO.onlineOfflineCheck']").filter(
			...
			return saveOrUpdateForm(
					...
					'NewWaterConnectionForm.html?PrintReport', 'save');
		} else if ($("input:radio[name='offlineDTO.onlineOfflineCheck']")
				.filter(":checked").val() == 'Y') {
			return saveOrUpdateForm(
					element,
					"Your application for new Water connection saved successfully!",
					'NewWaterConnectionForm.html?redirectToPay', 'save');
		} else {
			return saveOrUpdateForm(
					element,
					"Your application for new Water connection saved successfully!",
...
function getChecklistAndCharges(element) {
	...
	if ($("#propOutStanding").val() == 'Y' && $("#bplNo").val() == 'N') {
		errorList
				.push(getLocalMessage('Dues Exist against the Property Number Could Not Proceed For New Connection'));
	...
	if (duplicate != '') {
		...
		if (ccnNo != '') {
			...
			if (response.csIdn != 0) {
				...
				var errorList = [];
				errorList.push(getLocalMessage('water.bill.search'));
				displayErrorsOnPage(errorList);
			}
		}
...
function validateBhagirathiConnection() {
	...
	if (bplNo != '') {
		...
		if (bplNo == 'Y') {
			if (withBhagiRathi != '') {
				...
				if (bhagiRathi != '0.5') {
					var errorList = [];
					errorList
							.push(getLocalMessage('water.select.conn.size.incurr.charge'));
					displayErrorsOnPage(errorList);
				}
			}
		}
		...
		var errorList = [];
		errorList.push(getLocalMessage('water.select.bagh.conn.type'));
		displayErrorsOnPage(errorList);
	}
}
...
function fnValidatePAN(Obj) {
	...
	if (Obj != "") {
		...
		if (ObjVal.search(panPat) == -1) {
			errorList.push(getLocalMessage('water.invalid.pan'));
			/* $('#advPanno').val(""); */
	...
	if(propNoOptional != null && propNoOptional == 'Y'){
		if(chkConsumer == true){
			if (propertyNo == "" || propertyNo == '0' || propertyNo == undefined) {
				errorList
						.push(getLocalMessage('water.select.prop.num'));
			}
			if (csOname == '') {
...
/*	if (bplNo == '') {
	...
	if (!chkConsumer) {
		...
		if (csFirstName == '') {
			errorList.push(getLocalMessage('water.ConName'));
		}
		if (csAddress1 == '') {
			errorList.push(getLocalMessage('water.validation.ownerAddress'));
		}
		if (csPinCode == '') {
			errorList.push(getLocalMessage('water.pincode'));
		}
		if (csMobileNo == '') {
			errorList.push(getLocalMessage('water.mobile'));
		}
		if(sudaEnv == 'Y'){
			if (cSFatherGuardianName == '') {
				errorList.push(getLocalMessage('enter.father/guardianName'));
			}
			if (csDistrict == '' && csDistrict =='0') {
	...
	if (!chk) {
		...
		if (billingAreaName == '') {
			errorList.push(getLocalMessage('water.validation.billingArea'));
		}
		if (billingPinCode == '') {
			errorList.push(getLocalMessage('water.validation.billingPincode'));
		}
		if(sudaEnv == 'Y'){
	...
	if ($("#typeOfApplication").val() == 'T') {
		...
		if (fromdate == '') {
			errorList.push(getLocalMessage('water.select.from.period'));
		}
		if (todate == '') {
			errorList.push(getLocalMessage('water.select.to.period'));
		}
	...
	if (trmGroup1 == 0) {
		...
		if ($('#trmGroup1 option:selected').attr('code') == "H") {
			...
			if (restaurantNumber == '') {
				errorList.push(getLocalMessage('water.enter.tables'));
			}
		}
	}
	...
	/*
	 * var csCcnsize = $("#csCcnsize").val(); if (csCcnsize == 0) {
	 * errorList.push(getLocalMessage('Please Select Conection Size')); }
	 */

	if ($("input[name='csmrInfo.csPtype']").attr('value') == 'U') {
		var plumber = $("#plumber").val();
		if (plumber == '') {
			errorList.push(getLocalMessage('water.select.plumb.name'));
		}
	} else {
		...
		$("#tbl2 tbody tr")
				.each(
						function(i) {
							...
							if (consumerNo == '') {
								errorList
										.push(getLocalMessage('Please Select Conection No. For Entry No.')
												+ (i + 1));
							}
						});
	}
