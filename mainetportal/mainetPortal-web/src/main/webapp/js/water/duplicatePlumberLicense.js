...
function savePlRenwData(element) {
	...
	if ($("input:radio[name='offlineDTO.onlineOfflineCheck']").filter(
		...
		return saveOrUpdateForm(element,
				...
				'DuplicatePlumberLicense.html?PrintReport',
				'saveDuplicatePlumberLicense');
	} else if ($("input:radio[name='offlineDTO.onlineOfflineCheck']").filter(
			":checked").val() == 'Y') {
		return saveOrUpdateForm(element,
				"Your application for Duplicate License saved successfully!",
				'DuplicatePlumberLicense.html?redirectToPay',
				'saveDuplicatePlumberLicense');
	} else {
		return saveOrUpdateForm(element,
				"Your application for Duplicate License saved successfully!",
				'CitizenHome.html', 'saveDuplicatePlumberLicense');
	}
...
function viewLicenseDetails(obj) {
	...
	var theForm = '#' + formName;
	var request = __serializeForm(theForm);
	var url = 'DuplicatePlumberLicense.html?getLicApplicantDet';
