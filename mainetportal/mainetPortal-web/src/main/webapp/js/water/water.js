...
function saveForm(element) {
	...
	//	{
		//return saveOrUpdateForm(element,"Your application saved Successfully!", 'CitizenHome.html', 'saveform');	
...
function saveAndPayFordashboard(element)
{
	...
	return saveOrUpdateForm(element, '','CitizenHome.html','redirectToPay');
}
		...
		if (errorList.length == 0) {
			...
			data = __serializeForm(theForm);
				...
				if (responseObj.status == 'N') {
					errorList.push('No record found for connection no. '+conNum);
					$("#conName").val('');
					 $("#oldAdharNo").val('');
					 $("#conCat").val('');
					 $("#conType").val('');
